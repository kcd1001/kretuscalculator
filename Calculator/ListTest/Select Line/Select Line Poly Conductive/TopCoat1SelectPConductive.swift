...
struct TopCoat1SelectPConductive: View {
    ...
    func checkTC2()  {
        if ChipPConductive.TCoat2Choice == 3 { //POLY HS
            PAPUTC2.covRate = 560
        }
        else if ChipPConductive.TCoat2Choice == 4 { //POLY HP CLEAR GLOSS
            PAPUTC2.covRate = 575
        }
        else if ChipPConductive.TCoat2Choice == 5 { //POLY HP CLEAR SATIN
            PAPUTC2.covRate = 675
        }
        else {
            PAPUTC2.covRate = 750 
    ...
    func new_checkTC2() {
        switch ChipPConductive.TCoat2Choice {
        case 3:
    ...
    func checkTC1() { // change tc1 product coverage rate based off selection and chip size 
        if ChipPConductive.BroadcastSizeSelection == 0 {
            switch ChipPConductive.TCoat1Choice {
            case 3:
                PAPUTC1.covRate = 210
            default:
                PAPUTC1.covRate = 300
    ...
    if ChipPConductive.TCoat1Hardener == 0 {
        TCoat1PtACode = "EX-KPLY85AZ-01"
        TCoat1PtBCode = "EX-KPLY85B-01"
    }
    else if ChipPConductive.TCoat1Hardener == 1 {
...
//        PAPUTC1.covRate = 210
     ...
     //END OF TCOAT1
        ...
        if ChipPConductive.TCoat2Hardener == 0 {
            TCoat2PtACode = "EX-KPLY85AZ-01"
            TCoat2PtBCode = "EX-KPLY85B-01"
        }
        else if ChipPConductive.TCoat2Hardener == 1 {
...
//            }
        ...
        HStack {
            ...
            PolySelectInfo()
             ...
             .onChange(of: ChipPConductive.BroadcastSizeSelection) { _ in
               checkTC1()
             }
...
//            }
            HStack {
                 Picker(selection: $ChipPConductive.TCoat2Choice,
                     label: ZStack {
                         Text("\(TopCoat2Options[ChipPConductive.TCoat2Choice])")
                             .opacity(1)
                         Rectangle()
                             .fill(Color.gray)
                             .opacity(0.2)
                 ...
                 .background(Color.gray.opacity(0.2))
                 .cornerRadius(5)
                 .onChange(of: ChipPConductive.TCoat2Choice) { _ in
                   //checkTC1()
                   new_checkTC2()
                   checkTCoatCodes()
                 }
                 .onChange(of: ChipPConductive.TCoat2Hardener) { _ in
                   checkTCoatCodes()
                 }
...
//                }
            ...
            if ChipPConductive.TCoat2Choice == 1 { //Displays texture if it is selected
            Text("Texture:")
             .fontWeight(.bold)
                Picker(selection: $ChipPConductive.texture1,
                    label: ZStack {
                        Text("\(TextureOptions[ChipPConductive.texture1])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
...
//                }
            ...
            .padding()
                ...
                if ChipPConductive.TCoat2Choice == 1 { //Displays TC2 products if a second tc is chosen
                    VStack {
                        Text("Top Coat 2")
                            .bold()
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $ChipPConductive.TCoatWaste, formatter: NumberFormatter())
        ...
        .padding()
        if ChipPConductive.TCoat2Choice != 0 {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + quant(product: PAPUTC2) + ChipPConductive.TCoatWaste) kit(s)")
        }
        }
        else {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + ChipPConductive.TCoatWaste) kit(s)")
        }
        .padding()
        }
        }
...
struct TopCoat1SelectPConductive_Previews: PreviewProvider {
