...
struct TopCoat1SelectOMG: View {
    ...
    func checkTC2()  {
        if ChipOMG.TCoat2Choice == 3 { //POLY HS
            PAPUTC2.covRate = 560
        }
        else if ChipOMG.TCoat2Choice == 4 { //POLY HP CLEAR GLOSS
            PAPUTC2.covRate = 575
        }
        else if ChipOMG.TCoat2Choice == 5 { //POLY HP CLEAR SATIN
            PAPUTC2.covRate = 675
        }
        else {
            PAPUTC2.covRate = 750 
    ...
    func checkTC1() { // change tc1 product coverage rate based off selection and chip size 
        if ChipOMG.BroadcastSizeSelection == 0 {
            switch ChipOMG.TCoat1Choice {
            case 3:
                PAPUTC1.covRate = 210
            default:
                PAPUTC1.covRate = 300
...
//            }
        ...
        HStack {
            ...
            ChipTopCoatInfo()
             ...
             .onChange(of: ChipOMG.TCoat1Hardener) { _ in
               checkTCoatCodes()
             }
             .onChange(of: ChipOMG.BroadcastSizeSelection) { _ in
               checkTC1()
             }
...
//            }
            HStack {
                 Picker(selection: $ChipOMG.TCoat2Choice,
                     label: ZStack {
                         Text("\(TopCoat2Options[ChipOMG.TCoat2Choice])")
                             .opacity(1)
                         Rectangle()
                             .fill(Color.gray)
                             .opacity(0.2)
                 ...
                 .background(Color.gray.opacity(0.2))
                 .cornerRadius(5)
                 .onChange(of: ChipOMG.TCoat2Choice) { _ in
                   //checkTC1()
                   new_checkTC2()
                   checkTCoatCodes()
                 }
                 .onChange(of: ChipOMG.TCoat2Hardener) { _ in
                   checkTCoatCodes()
                 }
...
//                }
            ...
            if ChipOMG.TCoat2Choice == 1 { //Displays texture if it is selected
            Text("Texture:")
             .fontWeight(.bold)
                Picker(selection: $ChipOMG.texture1,
                    label: ZStack {
                        Text("\(TextureOptions[ChipOMG.texture1])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
...
//                }
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $ChipOMG.TCoatWaste, formatter: NumberFormatter())
        ...
        .padding()
        if ChipOMG.TCoat2Choice != 0 {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + quant(product: PAPUTC2) + ChipOMG.TCoatWaste) kit(s)")
        }
        }
        else {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + ChipOMG.TCoatWaste) kit(s)")
        }
        .padding()
        }
        }
...
struct TopCoat1SelectOMG_Previews: PreviewProvider {
