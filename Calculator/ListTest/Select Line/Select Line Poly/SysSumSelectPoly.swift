...
struct SysSumPCBCSelectPoly: View {
...
//                }
        ...
        if Chip.isMVR == true {
            VStack {
                ...
                HStack{ // Part A
                Text("\(typesACodes[ChipPoly.MVRPtA])")
                    .font(.caption)
                Spacer()
                Text("TSE Part A: \(typesA[ChipPoly.MVRPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipPoly.MVRWaste)")
                    .font(.caption)
                }
                HStack{ // Part B
                Text("\(typesBCodes[ChipPoly.MVRPtB])")
                    .font(.caption)
                Spacer()
                Text("TSE Part B: \(typesB[ChipPoly.MVRPtB]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipPoly.MVRWaste)")
                    .font(.caption)
                }
                if ChipPoly.MVRTSColorant != 0 {
                HStack{ // Colorant
                Text("\(TSColorantCodes[ChipPoly.MVRTSColorant])")
                    .font(.caption)
                Spacer()
                Text("TSE Colorant:  \(TSColorantChoices[ChipPoly.MVRTSColorant]), 16 oz")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipPoly.MVRWaste)")
                    .font(.caption)
...
struct SysSumPCBCSelectPoly_Previews: PreviewProvider {
