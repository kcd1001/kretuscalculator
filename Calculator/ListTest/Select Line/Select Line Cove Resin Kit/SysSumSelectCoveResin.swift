...
struct SysSumPCBCSelectCoveResin: View {
            ...
            VStack { // Start of coat summary
                ...
                HStack {
                    Text("\(kitCodes[ChipCoveResin.PCoatPtA])")
                        .font(.caption)
                    Spacer()
                    Text("Select Line Kit - \(kitTypes[ChipCoveResin.PCoatPtA])")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: selectTSA_TS) + ChipCoveResin.PCoatWaste)")
                        .font(.caption)
                }
...
//                }
        ...
        if Chip.isMVR == true {
            VStack {
                ...
                HStack{ // Part A
                Text("\(typesACodes[ChipCoveResin.MVRPtA])")
                    .font(.caption)
                Spacer()
                Text("TSE Part A: \(typesA[ChipCoveResin.MVRPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipCoveResin.MVRWaste)")
                    .font(.caption)
                }
                HStack{ // Part B
                Text("\(typesBCodes[ChipCoveResin.MVRPtB])")
                    .font(.caption)
                Spacer()
                Text("TSE Part B: \(typesB[ChipCoveResin.MVRPtB]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipCoveResin.MVRWaste)")
                    .font(.caption)
                }
                if ChipCoveResin.MVRTSColorant != 0 {
                HStack{ // Colorant
                Text("\(TSColorantCodes[ChipCoveResin.MVRTSColorant])")
                    .font(.caption)
                Spacer()
                Text("TSE Colorant:  \(TSColorantChoices[ChipCoveResin.MVRTSColorant]), 16 oz")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSMvr) + ChipCoveResin.MVRWaste)")
                    .font(.caption)
        ...
        VStack { // Start of coat summary
            ...
            HStack {
                Text("\(kitCodes[ChipCoveResin.BCoatPtA])")
                    .font(.caption)
                Spacer()
                Text("Kretus Select Poly Kit - \(kitTypes[ChipCoveResin.BCoatPtA])")
                    .font(.caption)
                Spacer()
...
struct SysSumPCBCSelectCoveResin_Previews: PreviewProvider {
