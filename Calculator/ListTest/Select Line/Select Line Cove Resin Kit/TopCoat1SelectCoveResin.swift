...
struct TopCoat1SelectCoveResin: View {
    ...
    func checkTC2()  {
        if ChipCoveResin.TCoat2Choice == 3 { //POLY HS
            PAPUTC2.covRate = 560
        }
        else if ChipCoveResin.TCoat2Choice == 4 { //POLY HP CLEAR GLOSS
            PAPUTC2.covRate = 575
        }
        else if ChipCoveResin.TCoat2Choice == 5 { //POLY HP CLEAR SATIN
            PAPUTC2.covRate = 675
        }
        else {
            PAPUTC2.covRate = 750 
    ...
    func new_checkTC2() {
        switch ChipCoveResin.TCoat2Choice {
        case 3:
    ...
    func checkTC1() { // change tc1 product coverage rate based off selection and chip size 
        if ChipCoveResin.BroadcastSizeSelection == 0 {
            switch ChipCoveResin.TCoat1Choice {
            case 3:
                PAPUTC1.covRate = 210
            default:
                PAPUTC1.covRate = 300
    ...
    if ChipCoveResin.TCoat1Hardener == 0 {
        TCoat1PtACode = "EX-KPLY85AZ-01"
        TCoat1PtBCode = "EX-KPLY85B-01"
    }
    else if ChipCoveResin.TCoat1Hardener == 1 {
...
//        PAPUTC1.covRate = 210
     ...
     //END OF TCOAT1
        ...
        if ChipCoveResin.TCoat2Hardener == 0 {
            TCoat2PtACode = "EX-KPLY85AZ-01"
            TCoat2PtBCode = "EX-KPLY85B-01"
        }
        else if ChipCoveResin.TCoat2Hardener == 1 {
...
//            }
        ...
        HStack {
            ...
            ChipTopCoatInfo()
             ...
             .onChange(of: ChipCoveResin.TCoat1Hardener) { _ in
               checkTCoatCodes()
             }
             .onChange(of: ChipCoveResin.BroadcastSizeSelection) { _ in
               checkTC1()
             }
...
//            }
            HStack {
                 Picker(selection: $ChipCoveResin.TCoat2Choice,
                     label: ZStack {
                         Text("\(TopCoat2Options[ChipCoveResin.TCoat2Choice])")
                             .opacity(1)
                         Rectangle()
                             .fill(Color.gray)
                             .opacity(0.2)
                 ...
                 .background(Color.gray.opacity(0.2))
                 .cornerRadius(5)
                 .onChange(of: ChipCoveResin.TCoat2Choice) { _ in
                   //checkTC1()
                   new_checkTC2()
                   checkTCoatCodes()
                 }
                 .onChange(of: ChipCoveResin.TCoat2Hardener) { _ in
                   checkTCoatCodes()
                 }
...
//                }
            ...
            if ChipCoveResin.TCoat2Choice == 1 { //Displays texture if it is selected
            Text("Texture:")
             .fontWeight(.bold)
                Picker(selection: $ChipCoveResin.texture1,
                    label: ZStack {
                        Text("\(TextureOptions[ChipCoveResin.texture1])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
...
//                }
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $ChipCoveResin.TCoatWaste, formatter: NumberFormatter())
        ...
        .padding()
        if ChipCoveResin.TCoat2Choice != 0 {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + quant(product: PAPUTC2) + ChipCoveResin.TCoatWaste) kit(s)")
        }
        }
        else {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + ChipCoveResin.TCoatWaste) kit(s)")
        }
        .padding()
        }
        }
...
struct TopCoat1SelectCoveResin_Previews: PreviewProvider {
