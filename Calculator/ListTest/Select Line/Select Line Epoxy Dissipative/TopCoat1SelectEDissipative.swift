...
struct TopCoat1SelectEDissipative: View {
    ...
    func checkTC2()  {
        if ChipEDissipative.TCoat2Choice == 3 { //POLY HS
            PAPUTC2.covRate = 560
        }
        else if ChipEDissipative.TCoat2Choice == 4 { //POLY HP CLEAR GLOSS
            PAPUTC2.covRate = 575
        }
        else if ChipEDissipative.TCoat2Choice == 5 { //POLY HP CLEAR SATIN
            PAPUTC2.covRate = 675
        }
        else {
            PAPUTC2.covRate = 750 
    ...
    func new_checkTC2() {
        switch ChipEDissipative.TCoat2Choice {
        case 3:
    ...
    func checkTC1() { // change tc1 product coverage rate based off selection and chip size 
        if ChipEDissipative.BroadcastSizeSelection == 0 {
            switch ChipEDissipative.TCoat1Choice {
            case 3:
                PAPUTC1.covRate = 210
            default:
                PAPUTC1.covRate = 300
...
//            }
        ...
        HStack {
            ...
            ChipTopCoatInfo()
             ...
             .onChange(of: ChipEDissipative.TCoat1Hardener) { _ in
               checkTCoatCodes()
             }
             .onChange(of: ChipEDissipative.BroadcastSizeSelection) { _ in
               checkTC1()
             }
...
//            }
            HStack {
                 Picker(selection: $ChipEDissipative.TCoat2Choice,
                     label: ZStack {
                         Text("\(TopCoat2Options[ChipEDissipative.TCoat2Choice])")
                             .opacity(1)
                         Rectangle()
                             .fill(Color.gray)
                             .opacity(0.2)
                 ...
                 .background(Color.gray.opacity(0.2))
                 .cornerRadius(5)
                 .onChange(of: ChipEDissipative.TCoat2Choice) { _ in
                   //checkTC1()
                   new_checkTC2()
                   checkTCoatCodes()
                 }
                 .onChange(of: ChipEDissipative.TCoat2Hardener) { _ in
                   checkTCoatCodes()
                 }
...
//                }
            ...
            if ChipEDissipative.TCoat2Choice == 1 { //Displays texture if it is selected
            Text("Texture:")
             .fontWeight(.bold)
                Picker(selection: $ChipEDissipative.texture1,
                    label: ZStack {
                        Text("\(TextureOptions[ChipEDissipative.texture1])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
...
//                }
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $ChipEDissipative.TCoatWaste, formatter: NumberFormatter())
        ...
        .padding()
        if ChipEDissipative.TCoat2Choice != 0 {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + quant(product: PAPUTC2) + ChipEDissipative.TCoatWaste) kit(s)")
        }
        }
        else {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + ChipEDissipative.TCoatWaste) kit(s)")
        }
        .padding()
        }
        }
...
struct TopCoat1SelectEDissipative_Previews: PreviewProvider {
