...
var metallicTSA = stats(product: "Top Shelf Epoxy Part A", covRate: 400, MixRat: 1)
var metallicTSB = stats(product: "Top Shelf Epoxy Part A", covRate: 400, MixRat: 0.5)
var metallicPigment = stats(product: "Top Shelf Epoxy Part A", covRate: 400, MixRat: 16)

struct TSMetallicPCoat: View {
    ...
    let pigmentChoices = ["No Pigment", "Antique Silver", "Black", "Cabernet", "California Gold", "Carbon", "Cherrywood", "Dark Blue", "Dark Gray", "Forest Green", "Gold Glitter", "Ivory", "Jade Green", "New Copper", "Olive", "Sedona", "Silver Glitter", "Sky Blue", "Sterling", "Sunset Gold", "Violet"]
 ...
 //           PCoattsMetallicInfo()
        ...
        HStack {
            Toggle(isOn: $tsMetallic.Coat1SolventCleaner, label: {
        ...
        .padding()
                ...
                Text("Top Shelf Epoxy® Part B:")
                    .fontWeight(.bold)
                Picker(selection: $tsMetallic.Coat1PtB,
                       label: ZStack {
                    Text("\(typesB[tsMetallic.Coat1PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
                ...
                Picker(selection: $tsMetallic.Coat1TSColorant,
                       label: ZStack {
                    Text("\(pigmentChoices[tsMetallic.Coat1TSColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $tsMetallic.Coat1Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: metallicTSA) + tsMetallic.Coat1Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct TSMetallicPCoat_Previews: PreviewProvider {
