...
struct EpoxyCoveChipBroadcast: View {
    ...
    func checkChipSize() {
        if CoveColorChip.BroadcastSizeSelection == 0 {
            Broadcast.covRate = 0.15
        }
        
        else if CoveColorChip.BroadcastSizeSelection == 1 {
            Broadcast.covRate = 0.15
        }
    }
    ...
    var body: some View {
        VStack {
            ...
            Text("Color Chip:")
             .fontWeight(.bold)
             Picker(selection: $CoveColorChip.BroadcastSizeSelection, //used for size of chips
                 label: ZStack {
                     Text("\(ChipSizes[CoveColorChip.BroadcastSizeSelection])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//                .cornerRadius(5)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $CoveColorChip.BroadcastWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: Broadcast) + CoveColorChip.BroadcastWaste) box(es)")
            }
            .padding()
        }
    }
...
struct EpoxyCoveChipBroadcast_Previews: PreviewProvider {
