...
struct EpoxyCoveCCoat: View {
    ...
    let TSColorantChoices = ["No Color (Unpigmented)","Beige", "Black", "Dark Gray", "Ench. Green", "Hand. Blue", "Latte", "Light Gray", "Medium Gray", "Mocha", "Safety Blue", "Safety Red", "Safety Yellow", "Shadow Gray", "Tan", "Tile Red", "White",]
        ...
        HStack {
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
                ...
                Picker(selection: $CoveColor.Coat3TSColorant,
                       label: ZStack {
                    Text("\(TSColorantChoices[CoveColor.Coat3TSColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: coveTSA) + CoveColor.Coat3Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct EpoxyCoveCCoat_Previews: PreviewProvider {
