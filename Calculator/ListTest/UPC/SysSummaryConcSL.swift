...
struct SysSummaryConcSL: View {
    ...
    var body: some View {
       ...
       else if upc.isPrimeCoat == true && upc.isTopCoat == false && (upcChoice.BCoatSelection != upcChoice.PCoatSelection) { // prime coat w/o top coat, unique values
            BCoatSwitchSL()
            PCoatSwitchSL()
       }
       else if upc.isPrimeCoat == true && upc.isTopCoat == false && (upcChoice.BCoatSelection == upcChoice.PCoatSelection) { // prime coat w/o top coat, non-unique values
            BCoatSwitchSL()
            PCoatSwitchSL()
       }
...
struct SysSummaryConcSL_Previews: PreviewProvider {
