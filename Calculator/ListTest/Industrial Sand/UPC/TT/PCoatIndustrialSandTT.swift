...
var TSA_SandTT = stats(product: "Top Shelf® Epoxy Part A:", covRate: 330, MixRat: 1)
var TSB_SandTT = stats(product: "Top Shelf® Epoxy Part B:", covRate: 330, MixRat: 0.5)
var TSColorant_SandTT = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 330, MixRat: 16)

struct PCoatIndustrialSandTT: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Choose Your Speed:")
             .fontWeight(.bold)
             Picker(selection: $SandTT.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandTT.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("UPC Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandTT.PCoatTSColorant,
                 label: ZStack {
                     Text("\(UPCColorantChoices[SandTT.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                HStack{
                    ...
                    Text("UPC Part A - RC/TT, 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSA_SandTT))")
                        .font(.caption)
                }
            
                HStack{
                    Text("\(typesBCodes[SandTT.PCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("UPC Part B - RC/TT \(typesB[SandTT.PCoatPtB]), 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSB_SandTT))")
                        .font(.caption)
                }
                ...
                if SandTT.PCoatTSColorant != 0
                {
                    HStack {
                        ...
                        Text("UPC Colorant:  \(UPCColorantChoices[SandTT.PCoatTSColorant]), 4 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_SandTT))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandTT.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_SandTT) + SandTT.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatIndustrialSandTT_Previews: PreviewProvider {
