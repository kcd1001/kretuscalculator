...
var TSA_SandDBRC = stats(product: "Top Shelf® Epoxy Part A:", covRate: 330, MixRat: 1)
var TSB_SandDBRC = stats(product: "Top Shelf® Epoxy Part B:", covRate: 330, MixRat: 0.5)
var TSColorant_SandDBRC = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 330, MixRat: 16)

struct PCoatIndustrialSandDBRC: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Choose Your Speed:")
             .fontWeight(.bold)
             Picker(selection: $SandDBRC.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandDBRC.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("UPC Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandDBRC.PCoatTSColorant,
                 label: ZStack {
                     Text("\(UPCColorantChoices[SandDBRC.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandDBRC.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_SandDBRC) + SandDBRC.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatIndustrialSandDBRC_Previews: PreviewProvider {
