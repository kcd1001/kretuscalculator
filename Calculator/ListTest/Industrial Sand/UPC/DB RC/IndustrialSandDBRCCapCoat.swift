...
var SandDBRC_TSAC3 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 120, MixRat: 1)
var SandDBRC_TSBC3 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 120, MixRat: 0.5)
var SandDBRC_TSColorantC3 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 120, MixRat: 16)

struct IndustrialSandDBRCCapCoat: View {
    ...
    ////            if SandDBRC.Coat3PtA == 0
    ////            {
    ////                TSAC3.covRate = 600
    ////                TSBC3.covRate = 600
    ////            }
    ////            // HP clear gloss
    ////            else if SandDBRC.Coat3PtA == 1
    ////            {
    ////                TSAC3.covRate = 575
    ////                TSBC3.covRate = 575
    ////            }
    ...
    func checkPCoatCodes() -> Void {
        ...
        if SandDBRC.Coat3PtA == 0 && SandDBRC.Coat3PtB == 0 {
            SandDBRC.PCoatPtACode = "EX-KPLY72AZ-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY72B-01"
        }
        else if SandDBRC.Coat3PtA == 0 && SandDBRC.Coat3PtB == 1 {
            SandDBRC.PCoatPtACode = "EX-KPLY72AF-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY72B-01"
            
        }
        else if SandDBRC.Coat3PtA == 1 && SandDBRC.Coat3PtB == 0 {
            SandDBRC.PCoatPtACode = "EX-KPLY85AZ-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY85B-01"
        }
        else if SandDBRC.Coat3PtA == 1 && SandDBRC.Coat3PtB == 1 {
            SandDBRC.PCoatPtACode = "EX-KPLY85AF-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY85B-01"
            
        }
        else if SandDBRC.Coat3PtA == 2 && SandDBRC.Coat3PtB == 0 {
            SandDBRC.PCoatPtACode = "EX-KPLY92AZ-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY92B-01"
            
        }
        else if SandDBRC.Coat3PtA == 2 && SandDBRC.Coat3PtB == 1 {
            SandDBRC.PCoatPtACode = "EX-KPLY92AF-01"
            SandDBRC.PCoatHardenerCode = "EX-KPLY92B-01"
    ...
    func checkPUCodes() -> Void
    {
        // POLY HS EZ
        if SandDBRC.Coat3PtA == 0 && SandDBRC.Coat3PtB == 0 {
            SandDBRC.PUPtACode = "EX-KPLYHSAZ-01"
            SandDBRC.PUHardenerCode = "EX-KPLYHSB-01"
        }
        // POLY HS FAST
        else if SandDBRC.Coat3PtA == 0 && SandDBRC.Coat3PtB == 1 {
            SandDBRC.PUPtACode = "EX-KPLYHSAF-01"
            SandDBRC.PUHardenerCode = "EX-KPLYHSB-01"
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 3 Product:")
                    .fontWeight(.bold)
                Picker(selection: $SandDBRC.Coat3ProductType,
                       label: ZStack {
                    Text("\(products[SandDBRC.Coat3ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            HStack {
                Toggle(isOn: $SandDBRC.Coat3MatAdd, label: {
            ...
            if SandDBRC.Coat3ProductType == 0
            {
                Text("Top Shelf Epoxy® Part A:")
                    .fontWeight(.bold)
                Picker(selection: $SandDBRC.Coat3PtA,
                       label: ZStack {
                    Text("\(typesA[SandDBRC.Coat3PtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
                ...
                Text("Top Shelf Epoxy® Part B:")
                    .fontWeight(.bold)
                Picker(selection: $SandDBRC.Coat3PtB,
                       label: ZStack {
                    Text("\(typesB[SandDBRC.Coat3PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                //             .onChange(of: SandDBRC.Coat3PtB) { _ in
                //                 CheckCoatChoices()
                //             }
                Text("Top Shelf Epoxy® Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $SandDBRC.Coat3TSColorant,
                       label: ZStack {
                    Text("\(TSColorantChoices[SandDBRC.Coat3TSColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            else if SandDBRC.Coat3ProductType == 1
            {
                HStack
                {
                    ...
                    .background(Color.gray.opacity(0.2))
                    .cornerRadius(5)
                    .onChange(of: SandDBRC.Coat3PtA) { _ in
                        checkPCoatCodes()
                    }
                    
                    Picker(selection: $SandDBRC.Coat3PtB,
                           label: ZStack {
                        Text("\(hardeners[SandDBRC.Coat3PtB])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.black)
                            .opacity(0.9)
            ...
            else
            {
                HStack{
                    VStack{
                        ...
                        Picker(selection: $SandDBRC.Coat3PtA,
                               label: ZStack {
                            Text("\(UPCTypes[SandDBRC.Coat3PtA])")
                                .opacity(1)
                            Rectangle()
                                .fill(Color.gray)
                                .opacity(0.2)
                ...
                HStack
                {
                    ...
                    Picker(selection: $SandDBRC.Coat3TSColorant,
                           label: ZStack {
                        Text("\(UPCColorantChoices[SandDBRC.Coat3TSColorant])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            if SandDBRC.Coat3ProductType != 0 && SandDBRC.Coat3ProductType != 3
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $SandDBRC.Coat3PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[SandDBRC.Coat3PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            //            Text("Texture:")
            //                .fontWeight(.bold)
            //            Picker(selection: $SandDBRC.Coat3Texture,
            //                   label: ZStack {
            //                Text("\(TextureOptions[SandDBRC.Coat3Texture])")
            //                    .opacity(1)
            //                Rectangle()
            //                    .fill(Color.black)
            //                    .opacity(0.9)
            ...
            //            .background(Color.gray.opacity(0.2))
            //            .cornerRadius(5)
            //            .onChange(of: SandDBRC.Coat3Texture) { _ in
            //                checkTexture()
            //            }
            ...
            VStack { // Start of coat summary
                ...
                if SandDBRC.Coat3ProductType == 0
                {
                    ...
                    if SandDBRC.Coat3PtA == 4 {
                        HStack{
                            Text("\(typesACodes[SandDBRC.Coat3PtA])")
                                .font(.caption)
                            Spacer()
                            Text("TSE Part A: Ench. Green, 1 gal")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: SandDBRC_TSAC3))")
                                .font(.caption)
                        }
                    }
                    else {
                        HStack{
                            Text("\(typesACodes[SandDBRC.Coat3PtA])")
                                .font(.caption)
                            Spacer()
                            Text("TSE Part A: \(typesA[SandDBRC.Coat3PtA]), 1 gal")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: SandDBRC_TSAC3))")
                                .font(.caption)
                    ...
                    HStack{
                        Text("\(typesBCodes[SandDBRC.Coat3PtB])")
                            .font(.caption)
                        Spacer()
                        Text("Top Shelf® Epoxy Part B: \(typesB[SandDBRC.Coat3PtB]), 1/2 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSAC3))")
                            .font(.caption)
                    }
                    if SandDBRC.Coat3TSColorant != 0 {
                        
                        HStack {
                            Text("\(TSColorantCodes[SandDBRC.Coat3TSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBRC.Coat3TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: SandDBRC_TSColorantC3))")
                                .font(.caption)
                        }
                ...
                else if SandDBRC.Coat3ProductType == 1
                {
                    HStack
                    {
                        Text("\(PCoatPtACode)")
                            .font(.caption)
                        Spacer()
                        Text("\(PATypes[SandDBRC.Coat3PtA]) - \(hardeners[SandDBRC.Coat3PtB]), 1 gal ")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSAC3))")
                            .font(.caption)
                    ...
                    HStack
                    {
                        Text("\(PCoatHardenerCode)")
                            .font(.caption)
                        Spacer()
                        Text("\(PATypes[SandDBRC.Coat3PtA]) Part B, 1 gal ")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSAC3))")
                            .font(.caption)
                ...
                else if SandDBRC.Coat3ProductType == 2
                {
                    ...
                    HStack
                    {
                        ...
                        if SandDBRC.Coat3PtA == 0
                        {
                            Text("\(PUTypes[SandDBRC.Coat3PtA]) - \(hardeners[SandDBRC.Coat3PtB]) Part A, 1 gal ")
                                .font(.caption)
                        }
                        // Poly HP Gloss
                        else if SandDBRC.Coat3PtA == 1
                        {
                            Text("\(PUTypes[SandDBRC.Coat3PtA]) Part A, 1/4 gal ")
                                .font(.caption)
                        }
                        // Poly HP Satin
                        else
                        {
                            Text("\(PUTypes[SandDBRC.Coat3PtA]) Part A, 1/2 gal ")
                                .font(.caption)
                        }
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSAC3))")
                    ...
                    HStack
                    {
                        ...
                        if SandDBRC.Coat3PtA == 0
                        {
                            Text("\(PUTypes[SandDBRC.Coat3PtA]) Part B, 1/2 gal ")
                                .font(.caption)
                        }
                        else
                        {
                            Text("\(PUTypes[SandDBRC.Coat3PtA]) Part B, 1 gal ")
                                .font(.caption)
                        }
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSAC3))")
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        HStack
                        {
                            Text("EX-BRCUVAP6-EA")
                                .font(.caption)
                            Spacer()
                            Text("UPC Part B - \(Text("\(typesB[SandDBRC.Coat3PtB])") ), 6#")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: SandDBRC_TSAC3 ))")
                                .font(.caption)
                    ...
                    if SandDBRC.Coat3TSColorant != 0
                    {
                        HStack
                        {
                            Text("\(UPCColorantCodes[SandDBRC.Coat3TSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("UPC Colorant - \(UPCColorantChoices[SandDBRC.Coat3TSColorant]), 4 oz")
                                .font(.caption)
                            Spacer()
                ...
                // poly colorant
                if SandDBRC.Coat3PColorant != 0 && (SandDBRC.Coat3ProductType != 0 || SandDBRC.Coat3ProductType != 3)
                {
                    HStack
                    {
                        Text("\(PolyColorantCodes[SandDBRC.Coat3PColorant])")
                            .font(.caption)
                        Spacer()
                        
                        Text("Poly Colorant - \(PolyColorantChoices[SandDBRC.Coat3PColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: SandDBRC_TSColorantC3))")
                            .font(.caption)
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $SandDBRC.Coat3Waste, formatter: NumberFormatter())
        ...
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: SandDBRC_TSAC3) + SandDBRC.Coat3Waste) kit(s)")
        }
        .padding()
    }
}
...
struct IndustrialSandDBRCCoat3_Previews: PreviewProvider {
