...
var TSABC2_SandDBRC = stats(product: "Top Shelf® Epoxy Part A:", covRate: 100, MixRat: 1)
var TSBBC2_SandDBRC = stats(product: "Top Shelf® Epoxy Part B:", covRate: 100, MixRat: 0.5)
var TSColorantBC2_SandDBRC = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 100, MixRat: 16)

struct BCoat2IndustrialSandDBRC: View {
    
 
    @EnvironmentObject var SandDBRC : ColorQuartzTSValues //Used for coat selections
    @EnvironmentObject var TSABC2_SandDBRC : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSBBC2_SandDBRC : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSColorantBC2_SandDBRC : stats //Used for system stats (mixRat,CovRate, etc)
...
//             }
            VStack { // Start of coat summary
                ...
                HStack{
                    ...
                    Text("UPC Part A - RC/TT, 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSABC2_SandDBRC))")
                        .font(.caption)
                }
                ...
                HStack{
                    ...
                    Text("UPC Part B - RC/TT \(typesB[SandDBRC.BCoatPtB]), 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSBBC2_SandDBRC))")
                        .font(.caption)
                }
                ...
                if SandDBRC.BCoatTSColorant != 0
                {
                        HStack {
                            ...
                            Text("UPC Colorant:  \(UPCColorantChoices[SandDBRC.BCoatTSColorant]), 4 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSColorantBC2_SandDBRC))")
                                .font(.caption)
                        }
                }
        }
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC2_SandDBRC) + SandDBRC.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoat2IndustrialSandDBRC_Previews: PreviewProvider {
