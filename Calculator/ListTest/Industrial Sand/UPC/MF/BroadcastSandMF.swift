...
//    var BroadcastQuartzTS = stats(product: "Color Chip:", covRate: 0.15, MixRat: 55)
    ...
    @EnvironmentObject var TSABC_SandMF : stats
    @EnvironmentObject var TSBBC_SandMF : stats
    @EnvironmentObject var TSColorantBC_SandMF : stats
    ...
    func updateCovRate() -> Void
    {
        ...
        // PolyU in cap coat
        else if SandMF.Coat4ProductType == 2
        {
            if SandMF.BroadcastSizeSelection == 0
            {
                SandRC_TSAC4.covRate = 120
        ...
        // UPC in cap coat
        else if SandMF.Coat4ProductType == 3
        {
            if SandMF.BroadcastSizeSelection == 0
            {
    ...
    var body: some View {
        VStack {
            ...
            Text("Industrial Sand:")
             .fontWeight(.bold)
             Picker(selection: $SandMF.BroadcastSizeSelection, //used for size of chips
                 label: ZStack {
                     Text("\(ChipSizes[SandMF.BroadcastSizeSelection])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .cornerRadius(5)
             .onChange(of: SandMF.BroadcastSizeSelection) { _ in
                 checkChipSize()
                 updateCovRate()
             }
             ...
             .onChange(of: SandMF.Coat4ProductType) { _ in
                ...
                Picker(selection: $SandMF.BroadcastSelection,
                    label: ZStack {
                        Text("\(textureChoices[SandMF.BroadcastSelection])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                HStack {
                    Text("Contact Distributor")
                        .font(.caption)
                    Spacer()
                    Text("Kretus® Approved \(ChipSizes[SandMF.BroadcastSizeSelection]) Industrial Sand")
                        .font(.caption)
                    Spacer()
                    Text("\(quantBroadcast(product: BroadcastSandMF))")
                        .font(.caption)
                }
        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandMF.BroadcastWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: BroadcastSandMF) + SandMF.BroadcastWaste) bag(s)")
            }
            .padding()
        }
    }
...
struct BroadcastSandMF_Previews: PreviewProvider {
