...
var TSA_SandRC = stats(product: "Top Shelf® Epoxy Part A:", covRate: 330, MixRat: 1)
var TSB_SandRC = stats(product: "Top Shelf® Epoxy Part B:", covRate: 330, MixRat: 0.5)
var TSColorant_SandRC = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 330, MixRat: 16)

struct PCoatIndustrialSandRC: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Choose Your Speed:")
             .fontWeight(.bold)
             Picker(selection: $SandRC.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandRC.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("UPC Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandRC.PCoatTSColorant,
                 label: ZStack {
                     Text("\(UPCColorantChoices[SandRC.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                HStack{
                    ...
                    Text("UPC Part A - RC/TT, 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSA_SandRC))")
                        .font(.caption)
                }
            
                HStack{
                    Text("\(typesBCodes[SandRC.PCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("UPC Part B - RC/TT \(typesB[SandRC.PCoatPtB]), 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSB_SandRC))")
                        .font(.caption)
                }
                ...
                if SandRC.PCoatTSColorant != 0
                {
                    HStack {
                        ...
                        Text("UPC Colorant:  \(UPCColorantChoices[SandRC.PCoatTSColorant]), 4 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_SandRC))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandRC.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_SandRC) + SandRC.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatIndustrialSandRC_Previews: PreviewProvider {
