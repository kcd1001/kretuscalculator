...
var SandRC_TSAC3 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 120, MixRat: 1)
var SandRC_TSBC3 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 120, MixRat: 0.5)
var SandRC_TSColorantC3 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 120, MixRat: 16)

struct IndustrialSandRCCapCoat: View {
...
//    }
    ...
    func checkPCoatCodes() -> Void {
        ...
        if SandRC.Coat3ProductType == 1
        {
            if SandRC.Coat3PtA == 0 && SandRC.Coat3PtB == 0 {
                SandRC.Coat3PtACode = "EX-KPLY72AZ-01"
                SandRC.Coat3PtBCode = "EX-KPLY72B-01"
            }
            else if SandRC.Coat3PtA == 0 && SandRC.Coat3PtB == 1 {
                SandRC.Coat3PtACode = "EX-KPLY72AF-01"
                SandRC.Coat3PtBCode = "EX-KPLY72B-01"
                
            }
            else if SandRC.Coat3PtA == 1 && SandRC.Coat3PtB == 0 {
                SandRC.Coat3PtACode = "EX-KPLY85AZ-01"
                SandRC.Coat3PtBCode = "EX-KPLY85B-01"
            }
            else if SandRC.Coat3PtA == 1 && SandRC.Coat3PtB == 1 {
                SandRC.Coat3PtACode = "EX-KPLY85AF-01"
                SandRC.Coat3PtBCode = "EX-KPLY85B-01"
                
            }
            else if SandRC.Coat3PtA == 2 && SandRC.Coat3PtB == 0 {
                SandRC.Coat3PtACode = "EX-KPLY92AZ-01"
                SandRC.Coat3PtBCode = "EX-KPLY92B-01"
                
            }
            else if SandRC.Coat3PtA == 2 && SandRC.Coat3PtB == 1 {
                SandRC.Coat3PtACode = "EX-KPLY92AF-01"
                SandRC.Coat3PtBCode = "EX-KPLY92B-01"
        ...
        else if SandRC.Coat3ProductType == 2
        {
            // POLY HS EZ
            if SandRC.Coat3PtA == 0 && SandRC.Coat3PtB == 0 {
                SandRC.Coat3PtACode = "EX-KPLYHSAZ-01"
                SandRC.Coat3PtBCode = "EX-KPLYHSB-01"
            }
            // POLY HS FAST
            else if SandRC.Coat3PtA == 0 && SandRC.Coat3PtB == 1 {
                SandRC.Coat3PtACode = "EX-KPLYHSAF-01"
                SandRC.Coat3PtBCode = "EX-KPLYHSB-01"
                
            }
            // POLY HP GLOSS
            else if SandRC.Coat3PtA == 1 {
                SandRC.Coat3PtACode = "EX-KPLYHPGA-01"
                SandRC.Coat3PtBCode = "EX-KPLYHPB-01"
            }
            // POLY HP SATIN
            else if SandRC.Coat3PtA == 2  {
                SandRC.Coat3PtACode = "EX-KPLYHPSA-01"
                SandRC.Coat3PtBCode = "EX-KPLYHPB-01"
        ...
        {
            // standard RC
            if SandRC.Coat3PtA == 1
            {
                if SandRC.Coat3PtB == 0
                {
                    SandRC.Coat3PtBCode = "EX-KUPCRCA6-EA"
                }
                
                else if SandRC.Coat3PtB == 1
                {
                    SandRC.Coat3PtBCode = "EX-KUPCRCZ6-EA"

                }
                else
                {
                    SandRC.Coat3PtBCode = "EX-KUPCRCF6-EA"
                }
            }
        }
    }
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 3 Product:")
                    .fontWeight(.bold)
                Picker(selection: $SandRC.Coat3ProductType,
                       label: ZStack {
                    Text("\(products[SandRC.Coat3ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
 ...
 //               SplashTSFloorInfo()
            ...
            HStack {
                Toggle(isOn: $SandRC.Coat3MatAdd, label: {
            ...
            Text("Top Shelf Epoxy® Part A:")
                .fontWeight(.bold)
            Picker(selection: $SandRC.Coat3PtA,
                   label: ZStack {
                Text("\(typesA[SandRC.Coat3PtA])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            Text("Top Shelf Epoxy® Part B:")
                .fontWeight(.bold)
            Picker(selection: $SandRC.Coat3PtB,
                   label: ZStack {
                Text("\(typesB[SandRC.Coat3PtB])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            //             .onChange(of: SandRC.Coat3PtB) { _ in
            //                 CheckCoatChoices()
            //             }
            Text("Top Shelf Epoxy® Colorant:")
                .fontWeight(.bold)
            Picker(selection: $SandRC.Coat3TSColorant,
                   label: ZStack {
                Text("\(TSColorantChoices[SandRC.Coat3TSColorant])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            else if SandRC.Coat3ProductType == 1
            {
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                .onChange(of: SandRC.Coat3PtA) { _ in
                    checkPCoatCodes()
                }
                
                Picker(selection: $SandRC.Coat3PtB,
                       label: ZStack {
                    Text("\(hardeners[SandRC.Coat3PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                        updateCovRate()
            ...
            else
            {
                ...
                HStack
                {
                    ...
                    Picker(selection: $SandRC.Coat3TSColorant,
                           label: ZStack {
                        Text("\(UPCColorantChoices[SandRC.Coat3TSColorant])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            if SandRC.Coat3ProductType != 0 && SandRC.Coat3ProductType != 3
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $SandRC.Coat3PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[SandRC.Coat3PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//            Text("Texture:")
//                .fontWeight(.bold)
//            Picker(selection: $SandRC.Coat3Texture,
//                   label: ZStack {
//                Text("\(TextureOptions[SandRC.Coat3Texture])")
//                    .opacity(1)
//                Rectangle()
//                    .fill(Color.black)
//                    .opacity(0.9)
...
//            .background(Color.gray.opacity(0.2))
//            .cornerRadius(5)
//            .onChange(of: SandRC.Coat3Texture) { _ in
//                checkTexture()
//            }
                ...
                if SandRC.Coat3PtA == 4 {
                    HStack{
                        ...
                        Text("TSE Part A: Ench. Green, 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: SandRC_TSAC3))")
                            .font(.caption)
                    }
                }
                else {
                ...
                if SandRC.Coat3TSColorant != 0 {
                        ...
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandRC.Coat3TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: SandRC_TSColorantC3))")
                                .font(.caption)
                        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandRC.Coat3Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: SandRC_TSAC3) + SandRC.Coat3Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct IndustrialSandRCCoat3_Previews: PreviewProvider {
