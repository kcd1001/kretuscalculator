...
var TSA_SandSL = stats(product: "Top Shelf® Epoxy Part A:", covRate: 330, MixRat: 1)
var TSB_SandSL = stats(product: "Top Shelf® Epoxy Part B:", covRate: 330, MixRat: 0.5)
var TSColorant_SandSL = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 330, MixRat: 16)

struct PCoatIndustrialSandSL: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Choose Your Speed:")
             .fontWeight(.bold)
             Picker(selection: $SandSL.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandSL.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("UPC Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandSL.PCoatTSColorant,
                 label: ZStack {
                     Text("\(UPCColorantChoices[SandSL.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                HStack{
                    ...
                    Text("UPC Part A - RC/TT, 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSA_SandSL))")
                        .font(.caption)
                }
            
                HStack{
                    Text("\(typesBCodes[SandSL.PCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("UPC Part B - RC/TT \(typesB[SandSL.PCoatPtB]), 6#")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSB_SandSL))")
                        .font(.caption)
                }
                ...
                if SandSL.PCoatTSColorant != 0
                {
                    HStack {
                        ...
                        Text("UPC Colorant:  \(UPCColorantChoices[SandSL.PCoatTSColorant]), 4 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_SandSL))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandSL.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_SandSL) + SandSL.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatIndustrialSandSL_Previews: PreviewProvider {
