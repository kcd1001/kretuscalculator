...
var TSABC_SandSG = stats(product: "Top Shelf® Epoxy Part A:", covRate: 50, MixRat: 1)
var TSBBC_SandSG = stats(product: "Top Shelf® Epoxy Part B:", covRate: 50, MixRat: 0.5)
var TSColorantBC_SandSG = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 50, MixRat: 16)

struct BCoatIndustrialSandSG: View {
    
 
    @EnvironmentObject var SandSG : ColorQuartzTSValues //Used for coat selections
    @EnvironmentObject var TSABC_SandSG : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSBBC_SandSG : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSColorantBC_SandSG : stats //Used for system stats (mixRat,CovRate, etc)
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $SandSG.BCoatPtA,
                label: ZStack {
                    Text("\(typesA[SandSG.BCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            .cornerRadius(5)
//            .onChange(of: SandSG.BCoatPtA) { _ in
//                CheckCoatChoices()
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandSG.BCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandSG.BCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandSG.BCoatPtB) { _ in
//                 CheckCoatChoices()
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandSG.BCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandSG.BCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandSG.BCoatTSColorant) { _ in
//                 CheckCoatChoices()
//             }
                ...
                HStack {
                    ...
                    HStack{
                        ...
                        Text("TSE Part A: \(typesA[SandSG.BCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSABC_SandSG))")
                            .font(.caption)
                    }
                ...
                if SandSG.BCoatTSColorant != 0
                {
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandSG.BCoatTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSColorantBC_SandSG))")
                                .font(.caption)
                        }
                }
                HStack
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandSG.BCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC_SandSG) + SandSG.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoatIndustrialSandSG_Previews: PreviewProvider {
