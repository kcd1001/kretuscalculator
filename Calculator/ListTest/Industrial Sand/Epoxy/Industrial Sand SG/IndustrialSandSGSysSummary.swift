...
struct IndustrialSandSGSysSummary: View {
    ...
    var BroadcastSandSG = statsBroadcast(product: "Color Quartz", covRate: 1, MixRat: 50)
    ...
    var body: some View {
        ...
        VStack
        {
            ...
            HStack{
                Text("\(typesACodes[SandSG.PCoatPtA])")
                    .font(.caption)
                Spacer()
                Text("TSE Part A: \(typesA[SandSG.PCoatPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSA_QuartzTS) + SandSG.PCoatWaste)")
                    .font(.caption)
            }
            
            HStack{
                Text("\(typesBCodes[SandSG.PCoatPtB])")
                    .font(.caption)
                Spacer()
                Text("Top Shelf® Epoxy Part B: \(typesB[SandSG.PCoatPtB]), 1/2 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSB_QuartzTS) + SandSG.PCoatWaste)")
                    .font(.caption)
            }
            ...
            if SandSG.PCoatSC
            {
                HStack{ // Solvent Cleaner
                    ...
                    Text("Solvent Cleaner, 32 oz")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSA_SandSG))")
                        .font(.caption)
                }
            }
            
            if SandSG.PCoatTSColorant != 0
            {
                HStack {
                    Text("\(TSColorantCodes[SandSG.BCoatTSColorant])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandSG.BCoatTSColorant]), 16 oz")
                        .font(.caption)
                    Spacer()
        ...
        if Chip.isMVR
        {
            ...
            VStack { // Start of coat summary
                ...
                HStack{
                    Text("\(typesACodes[SandSG.MVRPtA])")
                        .font(.caption)
                    Spacer()
                    Text("TSE Part A: \(typesA[SandSG.MVRPtA]), 1 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr) + SandSG.MVRWaste)")
                        .font(.caption)
                }
                
                HStack{
                    Text("\(typesBMVRCodes[SandSG.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesBMVR[SandSG.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr) + SandSG.MVRWaste)")
                        .font(.caption)
                }
                if SandSG.MVRTSColorant != 0 {
                    HStack {
                        Text("\(TSColorantCodes[SandSG.MVRTSColorant])")
                            .font(.caption)
                        Spacer()
                        Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandSG.MVRTSColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSMvr) + SandSG.MVRWaste)")
                            .font(.caption)
        ...
        VStack
        {
            ...
            HStack{
                Text("\(typesACodes[SandSG.BCoatPtA])")
                    .font(.caption)
                Spacer()
                Text("TSE Part A: \(typesA[SandSG.BCoatPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSABC_SandSG) + SandSG.BCoatWaste)")
                    .font(.caption)
            }
            
            HStack{
                Text("\(typesBCodes[SandSG.BCoatPtB])")
                    .font(.caption)
                Spacer()
                Text("Top Shelf® Epoxy Part B: \(typesB[SandSG.BCoatPtB]), 1/2 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSBBC_SandSG) + SandSG.BCoatWaste)")
                    .font(.caption)
            }
            if SandSG.BCoatTSColorant != 0
            {
                HStack {
                    Text("\(TSColorantCodes[SandSG.BCoatTSColorant])")
                        .font(.caption)
                    ...
                    Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandSG.BCoatTSColorant]), 16 oz")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSColorantBC_QuartzTS) + SandSG.BCoatWaste)")
                        .font(.caption)
                }
            }
            HStack
            {
                Text("Contact Distributor")
                    .font(.caption)
                Spacer()
                Text("Kretus® Approved 20-mesh Industrial Sand")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSBBC_SandSG))")
                    .font(.caption)
            }
        ...
        VStack { // Start of coat summary
            ...
            HStack {
                ...
                Text("Kretus® Approved \(ChipSizes[SandSG.BroadcastSizeSelection]) Industrial Sand")
                    .font(.caption)
                Spacer()
                Text("\(quantBroadcast(product: BroadcastSandSG) + SandSG.BroadcastWaste)")
                    .font(.caption)
            }
        }
        .background(Color(red:239.0/255.0, green: 243.0/255.0, blue: 244.0/250, opacity: 1.0))
...
struct IndustrialSandSGSysSummary_Previews: PreviewProvider {
