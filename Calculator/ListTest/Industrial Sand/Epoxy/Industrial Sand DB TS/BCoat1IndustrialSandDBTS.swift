...
var TSABC_SandDBTS = stats(product: "Top Shelf® Epoxy Part A:", covRate: 120, MixRat: 1)
var TSBBC_SandDBTS = stats(product: "Top Shelf® Epoxy Part B:", covRate: 120, MixRat: 0.5)
var TSColorantBC_SandDBTS = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 120, MixRat: 16)

struct BCoat1IndustrialSandDBTS: View {
    
 
    @EnvironmentObject var SandDBTS : ColorQuartzTSValues //Used for coat selections
    @EnvironmentObject var TSABC_SandDBTS : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSBBC_SandDBTS : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSColorantBC_SandEBTS : stats //Used for system stats (mixRat,CovRate, etc)
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $SandDBTS.BCoatPtA,
                label: ZStack {
                    Text("\(typesA[SandDBTS.BCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            .cornerRadius(5)
//            .onChange(of: SandDBTS.BCoatPtA) { _ in
//                CheckCoatChoices()
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.BCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandDBTS.BCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandDBTS.BCoatPtB) { _ in
//                 CheckCoatChoices()
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.BCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandDBTS.BCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandDBTS.BCoatTSColorant) { _ in
//                 CheckCoatChoices()
//             }
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        Text("\(typesACodes[SandDBTS.BCoatPtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[SandDBTS.BCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSABC_SandDBTS))")
                            .font(.caption)
                    }
            
                HStack{
                    Text("\(typesBCodes[SandDBTS.BCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[SandDBTS.BCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSBBC_SandDBTS))")
                        .font(.caption)
                }
                if SandDBTS.BCoatTSColorant != 0
                {
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBTS.BCoatTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSColorantBC_SandDBTS))")
                                .font(.caption)
                        }
                }
        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandDBTS.BCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC_SandDBTS) + SandDBTS.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoat1IndustrialSandDBTS_Previews: PreviewProvider {
