...
struct Broadcast2SandDBTS: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandDBTS.Broadcast2Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: BroadcastSandDBTS) + SandDBTS.Broadcast2Waste) bag(s)")
            }
            .padding()
        }
    }
...
struct Broadcast2SandDBTS_Previews: PreviewProvider {
