...
struct MVRCoatSandDBTS: View {
    ...
    var body: some View {
        VStack {
            ...
            .fontWeight(.bold)
            Picker(selection: $SandDBTS.MVRPtA,
                label: ZStack {
                    Text("\(typesA[SandDBTS.MVRPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            .cornerRadius(5)
//            .onChange(of: SandDBTS.PCoatPtA) { _ in
//                quant(product: TSA_TS)
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.MVRPtB,
                 label: ZStack {
                     Text("\(typesB[SandDBTS.MVRPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.MVRTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandDBTS.MVRTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandDBTS.PCoatTSColorant) { _ in
...
//             }
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        Text("\(typesACodes[SandDBTS.MVRPtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[SandDBTS.MVRPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                ...
                HStack{
                    Text("\(typesBCodes[SandDBTS.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[SandDBTS.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr))")
                        .font(.caption)
                }
                if SandDBTS.MVRTSColorant != 0 {
                        HStack {
                            Text("\(TSColorantCodes[SandDBTS.MVRTSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBTS.MVRTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSMvr))")
                                .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandDBTS.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSMvr) + SandDBTS.MVRWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct MVRCoatSandDBTS_Previews: PreviewProvider {
