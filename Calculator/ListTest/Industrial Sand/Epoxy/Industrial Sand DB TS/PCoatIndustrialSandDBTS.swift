...
var TSA_SandDBTS = stats(product: "Top Shelf® Epoxy Part A:", covRate: 450, MixRat: 1)
var TSB_SandDBTS = stats(product: "Top Shelf® Epoxy Part B:", covRate: 450, MixRat: 0.5)
var TSColorant_SandDBTS = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 450, MixRat: 16)

struct PCoatIndustrialSandDBTS: View {
    ...
    var body: some View {
        VStack {
            ...
            .padding()
            HStack {
                Toggle(isOn: $SandDBTS.PCoatSC, label: {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $SandDBTS.PCoatPtA,
                label: ZStack {
                    Text("\(typesA[SandDBTS.PCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandDBTS.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandDBTS.PCoatPtB) { _ in
//                quant(product: TSB)
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandDBTS.PCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandDBTS.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
                ...
                if SandDBTS.PCoatTSColorant != 0
                {
                    HStack {
                        Text("\(TSColorantCodes[SandDBTS.BCoatTSColorant])")
                            .font(.caption)
                        Spacer()
                        Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBTS.PCoatTSColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_SandTS))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandDBTS.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_SandTS) + SandDBTS.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatIndustrialSandDBTS_Previews: PreviewProvider {
