        ...
        VStack
        {
            ...
            HStack{
                ...
                Text("TSE Part A: \(typesA[SandDBTS.BCoatPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSABC_QuartzTS) + SandDBTS.BCoatWaste)")
                    .font(.caption)
            }
            ...
            HStack{
                ...
                Text("Top Shelf® Epoxy Part B: \(typesB[SandDBTS.BCoatPtB]), 1/2 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSBBC_QuartzTS) + SandDBTS.BCoatWaste)")
                    .font(.caption)
            }
            if SandDBTS.BCoatTSColorant != 0
            {
                HStack {
                    ...
                    Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBTS.BCoatTSColorant]), 16 oz")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSColorantBC_QuartzTS) + SandDBTS.BCoatWaste)")
                        .font(.caption)
                }
            }
...
struct SandBCoatDBTSSum_Previews: PreviewProvider {
