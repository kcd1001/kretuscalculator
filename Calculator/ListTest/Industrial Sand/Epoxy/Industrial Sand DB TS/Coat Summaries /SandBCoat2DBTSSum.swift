        ...
        VStack
        {
            ...
            HStack{
                ...
                Text("TSE Part A: \(typesA[SandDBTS.BCoat2PtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSABC_QuartzTS) + SandDBTS.BCoat2Waste)")
                    .font(.caption)
            }
            ...
            HStack{
                ...
                Text("Top Shelf® Epoxy Part B: \(typesB[SandDBTS.BCoat2PtB]), 1/2 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSBBC_QuartzTS) + SandDBTS.BCoat2Waste)")
                    .font(.caption)
            }
            if SandDBTS.BCoat2TSColorant != 0
            {
                HStack {
                    ...
                    Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandDBTS.BCoat2TSColorant]), 16 oz")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSColorantBC_QuartzTS) + SandDBTS.BCoat2Waste)")
                        .font(.caption)
                }
            }
...
struct SandBCoat2DBTSSum_Previews: PreviewProvider {
