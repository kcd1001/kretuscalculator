...
var TSABC_SandTG = stats(product: "Top Shelf® Epoxy Part A:", covRate: 44, MixRat: 1)
var TSBBC_SandTG = stats(product: "Top Shelf® Epoxy Part B:", covRate: 44, MixRat: 0.5)
var TSColorantBC_SandTG = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 44, MixRat: 16)

struct BCoatIndustrialSandTG: View {
    
 
    @EnvironmentObject var SandTG : ColorQuartzTSValues //Used for coat selections
    @EnvironmentObject var TSABC_SandTG : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSBBC_SandTG : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSColorantBC_SandTG : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var sf : SquareFeet //Square feet
    @EnvironmentObject var BroadcastSandTG : statsBroadcast
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $SandTG.BCoatPtA,
                label: ZStack {
                    Text("\(typesA[SandTG.BCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            .cornerRadius(5)
//            .onChange(of: SandTG.BCoatPtA) { _ in
//                CheckCoatChoices()
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandTG.BCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandTG.BCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTG.BCoatPtB) { _ in
//                 CheckCoatChoices()
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandTG.BCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandTG.BCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTG.BCoatTSColorant) { _ in
//                 CheckCoatChoices()
//             }
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        Text("\(typesACodes[SandTG.BCoatPtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[SandTG.BCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSABC_SandTG))")
                            .font(.caption)
                    }
            
                HStack{
                    Text("\(typesBCodes[SandTG.BCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[SandTG.BCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSBBC_SandTG))")
                        .font(.caption)
                }
                if SandTG.BCoatTSColorant != 0
                {
                        HStack {
                            Text("\(TSColorantCodes[SandTG.BCoatTSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[SandTG.BCoatTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSColorantBC_SandTG))")
                                .font(.caption)
                        }
                }
        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandTG.BCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC_SandTG) + SandTG.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoatIndustrialSandTG_Previews: PreviewProvider {
