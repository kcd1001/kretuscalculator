...
struct MVRCoatSandTS: View {
    ...
    let typesA = ["A Resin - Beige", "A Resin - Black", "A Resin - Clear", "A Resin - Dark Gray", "A Resin - Ench. Green", "A Resin - Hand. Blue",           "A Resin - Latte", "A Resin - Light Gray", "A Resin - Medium Gray", "A Resin - Mocha", "A Resin - Safety Blue", "A Resin - Safety Red",           "A Resin - Safety Yellow", "A Resin - Shadow Gray", "A Resin - Tan", "A Resin - Tile Red", "A Resin - White", "CR Resin - Clear",
                  "LG Resin - Clear", "Commercial Resin - Clear "]
    ...
    let typesACodes = ["EX-KTSARBG-01", "EX-KTSARBL-01", "EX-KTSARCL-01", "EX-KTSARDG-01", "EX-KTSAREG-01", "EX-KTSARHB-01", "EX-KTSARLT-01",                           "EX-KTSARLG-01", "EX-KTSARMG-01", "EX-KTSARMH-01", "EX-KTSARSB-01", "EX-KTSARSR-01", "EX-KTSARSY-01", "EX-KTSARSG-01",                            "EX-KTSARTN-01", "EX-KTSARTR-01", "EX-KTSARWH-01", "EX-KTSECRRC-01",
                       "Contact Distributor","FG-TSECMRCLR-01G"]
    ...
    var body: some View {
        VStack {
            ...
            .fontWeight(.bold)
            Picker(selection: $SandTS.MVRPtA,
                label: ZStack {
                    Text("\(typesA[SandTS.MVRPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            .cornerRadius(5)
//            .onChange(of: SandTS.PCoatPtA) { _ in
//                quant(product: TSA_TS)
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandTS.MVRPtB,
                 label: ZStack {
                     Text("\(typesB[SandTS.MVRPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTS.PCoatPtB) { _ in
//                quant(product: TSB_TS)
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandTS.MVRTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandTS.MVRTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTS.PCoatTSColorant) { _ in
...
//             }
            VStack { // Start of coat summary
                ...
                HStack{
                    Text("\(typesBCodes[SandTS.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[SandTS.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr))")
                        .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandTS.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSMvr) + SandTS.MVRWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct MVRCoatSandTS_Previews: PreviewProvider {
