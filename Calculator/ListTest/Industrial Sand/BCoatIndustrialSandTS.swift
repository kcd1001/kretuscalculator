...
var TSABC_SandTS = stats(product: "Top Shelf® Epoxy Part A:", covRate: 120, MixRat: 1)
var TSBBC_SandTS = stats(product: "Top Shelf® Epoxy Part B:", covRate: 120, MixRat: 0.5)
var TSColorantBC_SandTS = stats(product: "Top Shelf® Epoxy Colorant:", covRate: 120, MixRat: 16)

struct BCoatIndustrialSandTS: View {
    
 
    @EnvironmentObject var SandTS : ColorQuartzTSValues //Used for coat selections
    @EnvironmentObject var TSABC_SandTS : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSBBC_SandTS : stats //Used for system stats (mixRat,CovRate, etc)
    @EnvironmentObject var TSColorantBC_SandTS : stats //Used for system stats (mixRat,CovRate, etc)
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $SandTS.BCoatPtA,
                label: ZStack {
                    Text("\(typesA[SandTS.BCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            .cornerRadius(5)
//            .onChange(of: SandTS.BCoatPtA) { _ in
//                CheckCoatChoices()
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $SandTS.BCoatPtB,
                 label: ZStack {
                     Text("\(typesB[SandTS.BCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTS.BCoatPtB) { _ in
//                 CheckCoatChoices()
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $SandTS.BCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[SandTS.BCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: SandTS.BCoatTSColorant) { _ in
//                 CheckCoatChoices()
//             }
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        ...
                        Text("TSE Part A: \(typesA[SandTS.BCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSABC_SandTS))")
                            .font(.caption)
                    }
                ...
                HStack{
                    ...
                    Text("Top Shelf® Epoxy Part B: \(typesB[SandTS.BCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSBBC_SandTS))")
                        .font(.caption)
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $SandTS.BCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC_SandTS) + SandTS.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoatIndustrialSandTS_Previews: PreviewProvider {
