    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $QuartzDB.BCoatPtA,
                label: ZStack {
                    Text("\(typesA[QuartzDB.BCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.BCoatPtB,
                 label: ZStack {
                     Text("\(typesB[QuartzDB.BCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.BCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[QuartzDB.BCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        Text("\(typesACodes[QuartzDB.BCoatPtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[QuartzDB.BCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                ...
                HStack{
                    Text("\(typesBCodes[QuartzDB.BCoatPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzDB.BCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                ...
                if QuartzDB.BCoatTSColorant != 0
                {
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzDB.BCoatTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSColorantBC_QuartzDB))")
                                .font(.caption)
                        }
                }
        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzDB.BCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSABC_QuartzTS) + QuartzDB.BCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct BCoatQuartzDB_Previews: PreviewProvider {
