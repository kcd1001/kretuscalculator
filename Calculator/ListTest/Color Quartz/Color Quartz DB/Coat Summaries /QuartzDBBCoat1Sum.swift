        ...
        VStack
        {
            ...
            HStack{
                ...
                Text("TSE Part A: \(typesA[QuartzDB.BCoatPtA]), 1 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSABC_QuartzTS) + QuartzDB.BCoatWaste)")
                    .font(.caption)
            }
            ...
            HStack{
                ...
                Text("Top Shelf® Epoxy Part B: \(typesB[QuartzDB.BCoatPtB]), 1/2 gal")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: TSBBC_QuartzTS) + QuartzDB.BCoatWaste)")
                    .font(.caption)
            }
            if QuartzDB.BCoatTSColorant != 0
            {
                HStack {
                    ...
                    Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzDB.BCoatTSColorant]), 16 oz")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSColorantBC_QuartzTS) + QuartzDB.BCoatWaste)")
                        .font(.caption)
                }
            }
...
struct QuartzDBBCoat1Sum_Previews: PreviewProvider {
