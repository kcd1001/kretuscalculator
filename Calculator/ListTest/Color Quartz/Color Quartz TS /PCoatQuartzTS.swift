...
struct PCoatQuartzTS: View {
    ...
    @EnvironmentObject var TSColorant_QuartzTS : stats //Used for system stats (mixRat,CovRate, etc)
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $QuartzTS.PCoatPtA,
                label: ZStack {
                    Text("\(typesA[QuartzTS.PCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $QuartzTS.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[QuartzTS.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: QuartzTS.PCoatPtB) { _ in
//                quant(product: TSB)
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $QuartzTS.PCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[QuartzTS.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                HStack{
                    ...
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzTS.PCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSB_QuartzTS))")
                        .font(.caption)
                }
                ...
                if QuartzTS.PCoatTSColorant != 0
                {
                    HStack {
                        Text("\(TSColorantCodes[QuartzTS.BCoatTSColorant])")
                            .font(.caption)
                        Spacer()
                        Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzTS.PCoatTSColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_QuartzTS))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzTS.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_QuartzTS) + QuartzTS.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatQuartzTS_Previews: PreviewProvider {
