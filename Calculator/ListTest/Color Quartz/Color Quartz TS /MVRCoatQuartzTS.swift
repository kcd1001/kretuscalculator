...
struct MVRCoatQuartzTS: View {
    ...
    var body: some View {
        VStack {
            ...
            .fontWeight(.bold)
            Picker(selection: $QuartzTS.MVRPtA,
                label: ZStack {
                    Text("\(typesA[QuartzTS.MVRPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            .cornerRadius(5)
//            .onChange(of: QuartzTS.PCoatPtA) { _ in
//                quant(product: TSA_TS)
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $QuartzTS.MVRPtB,
                 label: ZStack {
                     Text("\(typesB[QuartzTS.MVRPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $QuartzTS.MVRTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[QuartzTS.MVRTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: QuartzTS.PCoatTSColorant) { _ in
...
//             }
            VStack { // Start of coat summary
                ...
                HStack{
                    Text("\(typesBCodes[QuartzTS.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzTS.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr))")
                        .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzTS.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSMvr) + QuartzTS.MVRWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct MVRCoatQuartzTS_Previews: PreviewProvider {
