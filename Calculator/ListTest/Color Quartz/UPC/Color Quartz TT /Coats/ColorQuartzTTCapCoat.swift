...
var QuartzTT_TSAC3 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 75, MixRat: 1)
var QuartzTT_TSBC3 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 75, MixRat: 0.5)
var QuartzTT_TSColorantC3 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 75, MixRat: 16)


struct ColorQuartzTTCapCoat: View {
    ...
    //        {
    //            QuartzTT_TSAC3.covRate = 120
    //            QuartzTT_TSBC3.covRate = 120
    //        }
    //        // change cov rate to 800 if polyA is chosen
    //        else if QuartzTT.Coat3ProductType == 1
    //        {
    //            QuartzTT_TSAC3.covRate = 160
    //            QuartzTT_TSBC3.covRate = 160
    //        }
    //        // else poly is chosen
    //        else
    //        {
    ...
    ////            if QuartzTT.Coat3PtA == 0
    ////            {
    ////                TSAC3.covRate = 600
    ////                TSBC3.covRate = 600
    ////            }
    ////            // HP clear gloss
    ////            else if QuartzTT.Coat3PtA == 1
    ////            {
    ////                TSAC3.covRate = 575
    ////                TSBC3.covRate = 575
    ////            }
    ...
    func checkPCoatCodes() -> Void {
        ...
        if QuartzTT.Coat3ProductType == 1
        {
            if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 0 {
                QuartzTT.Coat3PtACode = "EX-KPLY72AZ-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY72B-01"
            }
            else if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 1 {
                QuartzTT.Coat3PtACode = "EX-KPLY72AF-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY72B-01"
                
            }
            else if QuartzTT.Coat3PtA == 1 && QuartzTT.Coat3PtB == 0 {
                QuartzTT.Coat3PtACode = "EX-KPLY85AZ-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY85B-01"
            }
            else if QuartzTT.Coat3PtA == 1 && QuartzTT.Coat3PtB == 1 {
                QuartzTT.Coat3PtACode = "EX-KPLY85AF-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY85B-01"
                
            }
            else if QuartzTT.Coat3PtA == 2 && QuartzTT.Coat3PtB == 0 {
                QuartzTT.Coat3PtACode = "EX-KPLY92AZ-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY92B-01"
                
            }
            else if QuartzTT.Coat3PtA == 2 && QuartzTT.Coat3PtB == 1 {
                QuartzTT.Coat3PtACode = "EX-KPLY92AF-01"
                QuartzTT.Coat3PtBCode = "EX-KPLY92B-01"
        ...
        else if QuartzTT.Coat3ProductType == 2
        {
            // POLY HS EZ
            if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 0 {
                QuartzTT.Coat3PtACode = "EX-KPLYHSAZ-01"
                QuartzTT.Coat3PtBCode = "EX-KPLYHSB-01"
            }
            // POLY HS FAST
            else if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 1 {
                QuartzTT.Coat3PtACode = "EX-KPLYHSAF-01"
                QuartzTT.Coat3PtBCode = "EX-KPLYHSB-01"
                
            }
            // POLY HP GLOSS
            else if QuartzTT.Coat3PtA == 1 {
                QuartzTT.Coat3PtACode = "EX-KPLYHPGA-01"
                QuartzTT.Coat3PtBCode = "EX-KPLYHPB-01"
            }
            // POLY HP SATIN
            else if QuartzTT.Coat3PtA == 2  {
                QuartzTT.Coat3PtACode = "EX-KPLYHPSA-01"
                QuartzTT.Coat3PtBCode = "EX-KPLYHPB-01"
    ...
    func checkPUCodes() -> Void
    {
        // POLY HS EZ
        if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 0 {
            QuartzTT.PUPtACode = "EX-KPLYHSAZ-01"
            QuartzTT.PUHardenerCode = "EX-KPLYHSB-01"
        }
        // POLY HS FAST
        else if QuartzTT.Coat3PtA == 0 && QuartzTT.Coat3PtB == 1 {
            QuartzTT.PUPtACode = "EX-KPLYHSAF-01"
            QuartzTT.PUHardenerCode = "EX-KPLYHSB-01"
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 3 Product:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat3ProductType,
                       label: ZStack {
                    Text("\(products[QuartzTT.Coat3ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            HStack {
                Toggle(isOn: $QuartzTT.Coat3MatAdd, label: {
            ...
            if QuartzTT.Coat3ProductType == 0
            {
                Text("Top Shelf Epoxy® Part A:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat3PtA,
                       label: ZStack {
                    Text("\(typesA[QuartzTT.Coat3PtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
                ...
                Text("Top Shelf Epoxy® Part B:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat3PtB,
                       label: ZStack {
                    Text("\(typesB[QuartzTT.Coat3PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                //             .onChange(of: QuartzTT.Coat3PtB) { _ in
                //                 CheckCoatChoices()
                //             }
                Text("Top Shelf Epoxy® Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat3TSColorant,
                       label: ZStack {
                    Text("\(TSColorantChoices[QuartzTT.Coat3TSColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            else if QuartzTT.Coat3ProductType == 1
            {
                HStack
                {
                    ...
                    .background(Color.gray.opacity(0.2))
                    .cornerRadius(5)
                    .onChange(of: QuartzTT.Coat3PtA) { _ in
                        checkPCoatCodes()
                    }
                    
                    Picker(selection: $QuartzTT.Coat3PtB,
                           label: ZStack {
                        Text("\(hardeners[QuartzTT.Coat3PtB])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.black)
                            .opacity(0.9)
            ...
            if QuartzTT.Coat3ProductType != 0
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat3PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[QuartzTT.Coat3PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            //            Text("Texture:")
            //                .fontWeight(.bold)
            //            Picker(selection: $QuartzTT.Coat3Texture,
            //                   label: ZStack {
            //                Text("\(TextureOptions[QuartzTT.Coat3Texture])")
            //                    .opacity(1)
            //                Rectangle()
            //                    .fill(Color.black)
            //                    .opacity(0.9)
            ...
            //            .background(Color.gray.opacity(0.2))
            //            .cornerRadius(5)
            //            .onChange(of: QuartzTT.Coat3Texture) { _ in
            //                checkTexture()
            //            }
            ...
            VStack { // Start of coat summary
                ...
                if QuartzTT.Coat3ProductType == 0
                {
                    ...
                    if QuartzTT.Coat3PtA == 4 {
                        HStack{
                            Text("\(typesACodes[QuartzTT.Coat3PtA])")
                                .font(.caption)
                            Spacer()
                            Text("TSE Part A: Ench. Green, 1 gal")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzTT_TSAC3))")
                                .font(.caption)
                        }
                    }
                    else {
                        HStack{
                            Text("\(typesACodes[QuartzTT.Coat3PtA])")
                                .font(.caption)
                            Spacer()
                            Text("TSE Part A: \(typesA[QuartzTT.Coat3PtA]), 1 gal")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzTT_TSAC3))")
                                .font(.caption)
                    ...
                    HStack{
                        Text("\(typesBCodes[QuartzTT.Coat3PtB])")
                            .font(.caption)
                        Spacer()
                        Text("Top Shelf® Epoxy Part B: \(typesB[QuartzTT.Coat3PtB]), 1/2 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC3))")
                            .font(.caption)
                    }
                    if QuartzTT.Coat3TSColorant != 0 {
                        
                        HStack {
                            Text("\(TSColorantCodes[QuartzTT.Coat3TSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzTT.Coat3TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzTT_TSColorantC3))")
                                .font(.caption)
                        }
                ...
                else if QuartzTT.Coat3ProductType == 1
                {
                    HStack
                    {
                        Text("\(QuartzTT.Coat3PtACode)")
                            .font(.caption)
                        Spacer()
                        Text("\(PATypes[QuartzTT.Coat3PtA]) - \(hardeners[QuartzTT.Coat3PtB]), 1 gal ")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC3))")
                            .font(.caption)
                    ...
                    HStack
                    {
                        Text("\(QuartzTT.Coat3PtBCode)")
                            .font(.caption)
                        Spacer()
                        Text("\(PATypes[QuartzTT.Coat3PtA]) Part B, 1 gal ")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC3))")
                            .font(.caption)
                ...
                else
                {
                    ...
                    HStack
                    {
                        ...
                        if QuartzTT.Coat3PtA == 0
                        {
                            Text("\(PUTypes[QuartzTT.Coat3PtA]) - \(hardeners[QuartzTT.Coat3PtB]) Part A, 1 gal ")
                                .font(.caption)
                        }
                        // Poly HP Gloss
                        else if QuartzTT.Coat3PtA == 1
                        {
                            Text("\(PUTypes[QuartzTT.Coat3PtA]) Part A, 1/4 gal ")
                                .font(.caption)
                        }
                        // Poly HP Satin
                        else
                        {
                            Text("\(PUTypes[QuartzTT.Coat3PtA]) Part A, 1/2 gal ")
                                .font(.caption)
                        }
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC3))")
                    ...
                    HStack
                    {
                        ...
                        if QuartzTT.Coat3PtA == 0
                        {
                            Text("\(PUTypes[QuartzTT.Coat3PtA]) Part B, 1/2 gal ")
                                .font(.caption)
                        }
                        else
                        {
                            Text("\(PUTypes[QuartzTT.Coat3PtA]) Part B, 1 gal ")
                                .font(.caption)
                        }
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC3))")
                ...
                // poly colorant
                if QuartzTT.Coat3PColorant != 0 && QuartzTT.Coat3ProductType != 0
                {
                    HStack
                    {
                        Text("\(PolyColorantCodes[QuartzTT.Coat3PColorant])")
                            .font(.caption)
                        Spacer()
                        
                        Text("Poly Colorant - \(PolyColorantChoices[QuartzTT.Coat3PColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSColorantC3))")
                            .font(.caption)
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $QuartzTT.Coat3Waste, formatter: NumberFormatter())
        ...
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: QuartzTT_TSAC3) + QuartzTT.Coat3Waste) kit(s)")
        }
        .padding()
    }
}
...
struct QuartzTTCoat3_Previews: PreviewProvider {
