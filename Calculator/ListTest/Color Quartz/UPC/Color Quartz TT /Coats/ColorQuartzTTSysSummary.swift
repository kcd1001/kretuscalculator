...
struct ColorQuartzTTSysSummary: View {
    ...
    @EnvironmentObject var bCoatCoveQuartzTTA : stats2
    @EnvironmentObject var bCoatCoveQuartzTTB : stats2
    @EnvironmentObject var bCoatCoveQuartzTTColorant : stats2
    ...
    var BroadcastQuartzTT = statsBroadcast(product: "Color Quartz", covRate: 1, MixRat: 50)
        ...
        VStack
        {
            ...
            HStack{
                ...
                Text("UPC Part A - RC/TT, 6#")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: UPC_QuartzTT) + QuartzTT.PCoatWaste)")
                    .font(.caption)
            }
            
            HStack{
                Text("\(upcBCodes[QuartzTT.PCoatPtB])")
                    .font(.caption)
                Spacer()
                Text("UPC Part B RC/TT - \(typesB[QuartzTT.PCoatPtB]), 6#")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: UPC_QuartzTT) + QuartzTT.PCoatWaste)")
                    .font(.caption)
            ...
            if QuartzTT.PCoatTSColorant != 0
            {
                HStack {
                    Text("\(TSColorantCodes[QuartzTT.BCoatTSColorant])")
                        .font(.caption)
                    Spacer()
                    Text("UPC Colorant:  \(TSColorantChoices[QuartzTT.BCoatTSColorant]), 4 oz")
                        .font(.caption)
                    Spacer()
        ...
        VStack
        {
            ...
            HStack{
                ...
                Text("UPC Part A - RC/TT, 6#")
                    .font(.caption)
                Spacer()
                Text("\(quant(product: UPC_QuartzTT) + QuartzTT.BCoatWaste)")
                    .font(.caption)
            }
            
            HStack{
                Text("\(upcBCodes[QuartzTT.BCoatPtB])")
                    .font(.caption)
                Spacer()
        ...
        VStack { // Start of coat summary
            ...
            HStack {
                Text("Contact Distributor")
                    .font(.caption)
                Spacer()
                Text("Color Chip \(ChipSizes[QuartzTT.BroadcastSizeSelection]) - \(textureChoices[QuartzTT.BroadcastSelection]), 50#")
                    .font(.caption)
                Spacer()
                Text("\(quantBroadcast(product: BroadcastQuartzTT) + QuartzTT.BroadcastWaste)")
                    .font(.caption)
            }
        }
        .background(Color(red:239.0/255.0, green: 243.0/255.0, blue: 244.0/250, opacity: 1.0))
...
struct ColorQuartzTTSysSummary_Previews: PreviewProvider {
