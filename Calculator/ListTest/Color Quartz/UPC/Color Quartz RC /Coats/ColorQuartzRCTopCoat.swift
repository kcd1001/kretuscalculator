...
var QuartzRC_TSAC4 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 210, MixRat: 1)
var QuartzRC_TSBC4 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 210, MixRat: 0.5)
var QuartzRC_TSColorantC4 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 210, MixRat: 16)
...
struct ColorQuartzRCTopCoat: View {
    ...
    @EnvironmentObject var QuartzRC_TSAC4 : stats2
    @EnvironmentObject var QuartzRC_TSBC4 : stats2
    @EnvironmentObject var QuartzRC_TSColorantC4 : stats2
    ...
    func updateCovRate() -> Void
    {
        ...
        {
            QuartzRC_TSAC4.covRate = 750
            QuartzRC_TSBC4.covRate = 750
        }
        // else poly is chosen
        else
        {
            ...
            {
                QuartzRC_TSAC4.covRate = 560
                QuartzRC_TSBC4.covRate = 560
            }
            // HP clear gloss
            else if QuartzRC.Coat4PtA == 1
            {
                QuartzRC_TSAC4.covRate = 575
                QuartzRC_TSBC4.covRate = 575
            }

            // HP clear satin
            else
            {
                QuartzRC_TSAC4.covRate = 675
                QuartzRC_TSBC4.covRate = 675
            }
        }
    }
    func checkPCoatCodes() -> Void {
        
        if QuartzRC.Coat4ProductType == 1
        {
            if QuartzRC.Coat4PtA == 0 && QuartzRC.Coat4PtB == 0 {
                QuartzRC.Coat4PtACode = "EX-KPLY72AZ-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY72B-01"
            }
            else if QuartzRC.Coat4PtA == 0 && QuartzRC.Coat4PtB == 1 {
                QuartzRC.Coat4PtACode = "EX-KPLY72AF-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY72B-01"
                
            }
            else if QuartzRC.Coat4PtA == 1 && QuartzRC.Coat4PtB == 0 {
                QuartzRC.Coat4PtACode = "EX-KPLY85AZ-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY85B-01"
            }
            else if QuartzRC.Coat4PtA == 1 && QuartzRC.Coat4PtB == 1 {
                QuartzRC.Coat4PtACode = "EX-KPLY85AF-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY85B-01"
                
            }
            else if QuartzRC.Coat4PtA == 2 && QuartzRC.Coat4PtB == 0 {
                QuartzRC.Coat4PtACode = "EX-KPLY92AZ-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY92B-01"
                
            }
            else if QuartzRC.Coat4PtA == 2 && QuartzRC.Coat4PtB == 1 {
                QuartzRC.Coat4PtACode = "EX-KPLY92AF-01"
                QuartzRC.Coat4PtBCode = "EX-KPLY92B-01"
        ...
        else if QuartzRC.Coat4ProductType == 2
        {
            // POLY HS EZ
            if QuartzRC.Coat4PtA == 0 && QuartzRC.Coat4PtB == 0 {
                QuartzRC.Coat4PtACode = "EX-KPLYHSAZ-01"
                QuartzRC.Coat4PtBCode = "EX-KPLYHSB-01"
            }
            // POLY HS FAST
            else if QuartzRC.Coat4PtA == 0 && QuartzRC.Coat4PtB == 1 {
                QuartzRC.Coat4PtACode = "EX-KPLYHSAF-01"
                QuartzRC.Coat4PtBCode = "EX-KPLYHSB-01"
                
            }
            // POLY HP GLOSS
            else if QuartzRC.Coat4PtA == 1 {
                QuartzRC.Coat4PtACode = "EX-KPLYHPGA-01"
                QuartzRC.Coat4PtBCode = "EX-KPLYHPB-01"
            }
            // POLY HP SATIN
            else if QuartzRC.Coat4PtA == 2  {
                QuartzRC.Coat4PtACode = "EX-KPLYHPSA-01"
                QuartzRC.Coat4PtBCode = "EX-KPLYHPB-01"
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 4 Product:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzRC.Coat4ProductType,
                       label: ZStack {
                    Text("\(products[QuartzRC.Coat4ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                    checkPUCodes()
 ...
 //               SplashTSFloorInfo()
            ...
            HStack {
                Toggle(isOn: $QuartzRC.Coat4MatAdd, label: {
            ...
            Text("Top Shelf Epoxy® Part A:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat4PtA,
                   label: ZStack {
                Text("\(typesA[QuartzRC.Coat4PtA])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            Text("Top Shelf Epoxy® Part B:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat4PtB,
                   label: ZStack {
                Text("\(typesB[QuartzRC.Coat4PtB])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            //             .onChange(of: QuartzRC.Coat4PtB) { _ in
            //                 CheckCoatChoices()
            //             }
            Text("Top Shelf Epoxy® Colorant:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat4TSColorant,
                   label: ZStack {
                Text("\(TSColorantChoices[QuartzRC.Coat4TSColorant])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            else if QuartzRC.Coat4ProductType == 1
            {
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                .onChange(of: QuartzRC.Coat4PtA) { _ in
                    checkPCoatCodes()
                }
                
                Picker(selection: $QuartzRC.Coat4PtB,
                       label: ZStack {
                    Text("\(hardeners[QuartzRC.Coat4PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                        checkPUCodes()
            ...
            if QuartzRC.Coat4ProductType != 0
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzRC.Coat4PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[QuartzRC.Coat4PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            Text("Texture:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat4Texture,
                   label: ZStack {
                Text("\(TextureOptions[QuartzRC.Coat4Texture])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            .onChange(of: QuartzRC.Coat4Texture) { _ in
                checkTexture()
            }
            ...
            VStack { // Start of coat summary
                ...
                if QuartzRC.Coat4PtA == 4 {
                    HStack{
                        ...
                        Text("TSE Part A: Ench. Green, 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzRC_TSAC4))")
                            .font(.caption)
                    }
                }
                else {
                ...
                HStack
                    ...
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzRC.Coat4PtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzRC_TSBC4))")
                        .font(.caption)
                }
                if QuartzRC.Coat4TSColorant != 0
                    {
                        ...
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzRC.Coat4TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzRC_TSColorantC4))")
                                .font(.caption)
                        }
            ...
            else if QuartzRC.Coat4ProductType == 2
            {
                ...
                HStack
                {
                    ...
                    if QuartzRC.Coat4PtA == 0
                    {
                    Text("\(PUTypes[QuartzRC.Coat4PtA]) - \(hardeners[QuartzRC.Coat4PtB]) Part A, 1 gal ")
                        .font(.caption)
                    }
                    // Poly HP Gloss
                    else if QuartzRC.Coat4PtA == 1
                    {
                    Text("\(PUTypes[QuartzRC.Coat4PtA]) Part A, 1/4 gal ")
                        .font(.caption)
                    }
                    // Poly HP Satin
                    else
                    {
                        Text("\(PUTypes[QuartzRC.Coat4PtA]) Part A, 1/2 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzRC_TSAC4))")
                ...
                HStack
                {
                    ...
                    if QuartzRC.Coat4PtA == 0
                    {
                    Text("\(PUTypes[QuartzRC.Coat4PtA]) Part B, 1/2 gal ")
                        .font(.caption)
                    }
                    else
                    {
                        Text("\(PUTypes[QuartzRC.Coat4PtA]) Part B, 1 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzRC_TSAC4))")
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzRC.Coat4Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: QuartzRC_TSAC4) + QuartzRC.Coat4Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct QuartzRCTopCoat_Previews: PreviewProvider {
