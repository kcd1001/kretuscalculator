...
var QuartzTT_TSAC4 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 210, MixRat: 1)
var QuartzTT_TSBC4 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 210, MixRat: 0.5)
var QuartzTT_TSColorantC4 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 210, MixRat: 16)
...
struct ColorQuartzTTTopCoat: View {
    ...
    @EnvironmentObject var QuartzTT_TSAC4 : stats2
    @EnvironmentObject var QuartzTT_TSBC4 : stats2
    @EnvironmentObject var QuartzTT_TSColorantC4 : stats2
    ...
    func updateCovRate() -> Void
    {
        ...
        else if QuartzTT.Coat4ProductType == 1
        {
            QuartzTT_TSAC4.covRate = 750
            QuartzTT_TSBC4.covRate = 750
        }
        // else poly is chosen
        else
        {
            ...
            if QuartzTT.Coat4PtA == 0
            {
                QuartzTT_TSAC4.covRate = 560
                QuartzTT_TSBC4.covRate = 560
            }
            // HP clear gloss
            else if QuartzTT.Coat4PtA == 1
            {
                QuartzTT_TSAC4.covRate = 575
                QuartzTT_TSBC4.covRate = 575
            }

            // HP clear satin
            else
            {
                QuartzTT_TSAC4.covRate = 675
                QuartzTT_TSBC4.covRate = 675
            }
        }
    }
    ...
    func checkPCoatCodes() -> Void {
        ...
        if QuartzTT.Coat4PtA == 0 && QuartzTT.Coat4PtB == 0 {
            PCoatPtACode = "EX-KPLY72AZ-01"
            PCoatHardenerCode = "EX-KPLY72B-01"
        }
        else if QuartzTT.Coat4PtA == 0 && QuartzTT.Coat4PtB == 1 {
            PCoatPtACode = "EX-KPLY72AF-01"
            PCoatHardenerCode = "EX-KPLY72B-01"
            
        }
        else if QuartzTT.Coat4PtA == 1 && QuartzTT.Coat4PtB == 0 {
            PCoatPtACode = "EX-KPLY85AZ-01"
            PCoatHardenerCode = "EX-KPLY85B-01"
        }
        else if QuartzTT.Coat4PtA == 1 && QuartzTT.Coat4PtB == 1 {
            PCoatPtACode = "EX-KPLY85AF-01"
            PCoatHardenerCode = "EX-KPLY85B-01"
            
        }
        else if QuartzTT.Coat4PtA == 2 && QuartzTT.Coat4PtB == 0 {
            PCoatPtACode = "EX-KPLY92AZ-01"
            PCoatHardenerCode = "EX-KPLY92B-01"
            
        }
        else if QuartzTT.Coat4PtA == 2 && QuartzTT.Coat4PtB == 1 {
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 4 Product:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat4ProductType,
                       label: ZStack {
                    Text("\(products[QuartzTT.Coat4ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
 ...
 //               SplashTSFloorInfo()
            ...
            HStack {
                Toggle(isOn: $QuartzTT.Coat4MatAdd, label: {
            ...
            Text("Top Shelf Epoxy® Part A:")
                .fontWeight(.bold)
            Picker(selection: $QuartzTT.Coat4PtA,
                   label: ZStack {
                Text("\(typesA[QuartzTT.Coat4PtA])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            Text("Top Shelf Epoxy® Part B:")
                .fontWeight(.bold)
            Picker(selection: $QuartzTT.Coat4PtB,
                   label: ZStack {
                Text("\(typesB[QuartzTT.Coat4PtB])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            //             .onChange(of: QuartzTT.Coat4PtB) { _ in
            //                 CheckCoatChoices()
            //             }
            Text("Top Shelf Epoxy® Colorant:")
                .fontWeight(.bold)
            Picker(selection: $QuartzTT.Coat4TSColorant,
                   label: ZStack {
                Text("\(TSColorantChoices[QuartzTT.Coat4TSColorant])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            else if QuartzTT.Coat4ProductType == 1
            {
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                .onChange(of: QuartzTT.Coat4PtA) { _ in
                    checkPCoatCodes()
                }
                
                Picker(selection: $QuartzTT.Coat4PtB,
                       label: ZStack {
                    Text("\(hardeners[QuartzTT.Coat4PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            if QuartzTT.Coat4ProductType != 0
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzTT.Coat4PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[QuartzTT.Coat4PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
            ...
            Text("Texture:")
                .fontWeight(.bold)
            Picker(selection: $QuartzTT.Coat4Texture,
                   label: ZStack {
                Text("\(TextureOptions[QuartzTT.Coat4Texture])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            .onChange(of: QuartzTT.Coat4Texture) { _ in
                checkTexture()
            }
            ...
            VStack { // Start of coat summary
                ...
                if QuartzTT.Coat4ProductType == 0
                {
                if QuartzTT.Coat4PtA == 4 {
                    HStack{
                        Text("\(typesACodes[QuartzTT.Coat4PtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: Ench. Green, 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC4))")
                            .font(.caption)
                    }
                }
                else {
                    HStack{
                        Text("\(typesACodes[QuartzTT.Coat4PtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[QuartzTT.Coat4PtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSAC4))")
                            .font(.caption)
                ...
                HStack
                    {
                    Text("\(typesBCodes[QuartzTT.Coat4PtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzTT.Coat4PtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzTT_TSBC4))")
                        .font(.caption)
                }
                if QuartzTT.Coat4TSColorant != 0
                    {

                        HStack {
                            Text("\(TSColorantCodes[QuartzTT.Coat4TSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzTT.Coat4TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzTT_TSColorantC4))")
                                .font(.caption)
                        }
            ...
            else if QuartzTT.Coat4ProductType == 1
            {
                HStack
                {
                    Text("\(PCoatPtACode)")
                        .font(.caption)
                    Spacer()
                    Text("\(PATypes[QuartzTT.Coat4PtA]) - \(hardeners[QuartzTT.Coat4PtB]), 1 gal ")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzTT_TSAC4))")
                        .font(.caption)
                ...
                HStack
                {
                    Text("\(PCoatHardenerCode)")
                        .font(.caption)
                    Spacer()
                    Text("\(PATypes[QuartzTT.Coat4PtA]) Part B, 1 gal ")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzTT_TSAC4))")
                        .font(.caption)
            ...
            else if QuartzTT.Coat4ProductType == 2
            {
                ...
                HStack
                {
                    ...
                    if QuartzTT.Coat4PtA == 0
                    {
                    Text("\(PUTypes[QuartzTT.Coat4PtA]) - \(hardeners[QuartzTT.Coat4PtB]) Part A, 1 gal ")
                        .font(.caption)
                    }
                    // Poly HP Gloss
                    else if QuartzTT.Coat4PtA == 1
                    {
                    Text("\(PUTypes[QuartzTT.Coat4PtA]) Part A, 1/4 gal ")
                        .font(.caption)
                    }
                    // Poly HP Satin
                    else
                    {
                        Text("\(PUTypes[QuartzTT.Coat4PtA]) Part A, 1/2 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzTT_TSAC4))")
                ...
                HStack
                {
                    ...
                    if QuartzTT.Coat4PtA == 0
                    {
                    Text("\(PUTypes[QuartzTT.Coat4PtA]) Part B, 1/2 gal ")
                        .font(.caption)
                    }
                    else
                    {
                        Text("\(PUTypes[QuartzTT.Coat4PtA]) Part B, 1 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzTT_TSAC4))")
                ...
                // poly colorant
                if QuartzTT.Coat4PColorant != 0 && QuartzTT.Coat4ProductType != 0
                {
                    HStack
                    {
                        Text("\(PolyColorantCodes[QuartzTT.Coat4PColorant])")
                            .font(.caption)
                        Spacer()
                       
                        Text("Poly Colorant - \(PolyColorantChoices[QuartzTT.Coat4PColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzTT_TSColorantC4))")
                            .font(.caption)
                ...
                if QuartzTT.Coat4Texture != 0 {
                    if QuartzTT.Coat4Texture < 5 {
                        HStack {
                            Text("Contact Distributor")
                                .font(.caption)
                            Spacer()
                            Text("\(TextureOptions[QuartzTT.Coat4Texture]), 10#")
                                .font(.caption)
                            Spacer()
                    ...
                    if QuartzTT.Coat4Texture >= 5 {
                        HStack {
                            Text("Contact Distributor")
                                .font(.caption)
                            Spacer()
                            Text("\(TextureOptions[QuartzTT.Coat4Texture]), 32 oz")
                                .font(.caption)
                            Spacer()
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzTT.Coat4Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: QuartzTT_TSAC4) + QuartzTT.Coat4Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct QuartzTTTopCoat_Previews: PreviewProvider {
