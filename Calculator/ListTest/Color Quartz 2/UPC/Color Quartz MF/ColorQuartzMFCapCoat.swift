...
var QuartzMF_TSAC3 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 75, MixRat: 1)
var QuartzMF_TSBC3 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 75, MixRat: 0.5)
var QuartzMF_TSColorantC3 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 75, MixRat: 16)

struct ColorQuartzMFCapCoat: View {
    ...
    func checkPCoatCodes() -> Void {
        ...
        if QuartzMF.Coat3PtA == 0 && QuartzMF.Coat3PtB == 0 {
            QuartzMF.PCoatPtACode = "EX-KPLY72AZ-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY72B-01"
        }
        else if QuartzMF.Coat3PtA == 0 && QuartzMF.Coat3PtB == 1 {
            QuartzMF.PCoatPtACode = "EX-KPLY72AF-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY72B-01"
            
        }
        else if QuartzMF.Coat3PtA == 1 && QuartzMF.Coat3PtB == 0 {
            QuartzMF.PCoatPtACode = "EX-KPLY85AZ-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY85B-01"
        }
        else if QuartzMF.Coat3PtA == 1 && QuartzMF.Coat3PtB == 1 {
            QuartzMF.PCoatPtACode = "EX-KPLY85AF-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY85B-01"
            
        }
        else if QuartzMF.Coat3PtA == 2 && QuartzMF.Coat3PtB == 0 {
            QuartzMF.PCoatPtACode = "EX-KPLY92AZ-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY92B-01"
            
        }
        else if QuartzMF.Coat3PtA == 2 && QuartzMF.Coat3PtB == 1 {
            QuartzMF.PCoatPtACode = "EX-KPLY92AF-01"
            QuartzMF.PCoatHardenerCode = "EX-KPLY92B-01"
    ...
    func checkPUCodes() -> Void
    {
        // POLY HS EZ
        if QuartzMF.Coat3PtA == 0 && QuartzMF.Coat3PtB == 0 {
            QuartzMF.PUPtACode = "EX-KPLYHSAZ-01"
            QuartzMF.PUHardenerCode = "EX-KPLYHSB-01"
        }
        // POLY HS FAST
        else if QuartzMF.Coat3PtA == 0 && QuartzMF.Coat3PtB == 1 {
            QuartzMF.PUPtACode = "EX-KPLYHSAF-01"
            QuartzMF.PUHardenerCode = "EX-KPLYHSB-01"
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 3 Product:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzMF.Coat3ProductType,
                       label: ZStack {
                    Text("\(products[QuartzMF.Coat3ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
 ...
 //               SplashTSFloorInfo()
            ...
            HStack {
                Toggle(isOn: $QuartzMF.Coat3MatAdd, label: {
            ...
            Text("Top Shelf Epoxy® Part A:")
                .fontWeight(.bold)
            Picker(selection: $QuartzMF.Coat3PtA,
                   label: ZStack {
                Text("\(typesA[QuartzMF.Coat3PtA])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            Text("Top Shelf Epoxy® Part B:")
                .fontWeight(.bold)
            Picker(selection: $QuartzMF.Coat3PtB,
                   label: ZStack {
                Text("\(typesB[QuartzMF.Coat3PtB])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            //             .onChange(of: QuartzMF.Coat3PtB) { _ in
            //                 CheckCoatChoices()
            //             }
            Text("Top Shelf Epoxy® Colorant:")
                .fontWeight(.bold)
            Picker(selection: $QuartzMF.Coat3TSColorant,
                   label: ZStack {
                Text("\(TSColorantChoices[QuartzMF.Coat3TSColorant])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            else if QuartzMF.Coat3ProductType == 1
            {
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                .onChange(of: QuartzMF.Coat3PtA) { _ in
                    checkPCoatCodes()
                }
                
                Picker(selection: $QuartzMF.Coat3PtB,
                       label: ZStack {
                    Text("\(hardeners[QuartzMF.Coat3PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                        updateCovRate()
            ...
            if QuartzMF.Coat3ProductType != 0
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzMF.Coat3PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[QuartzMF.Coat3PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//            Text("Texture:")
//                .fontWeight(.bold)
//            Picker(selection: $QuartzMF.Coat3Texture,
//                   label: ZStack {
//                Text("\(TextureOptions[QuartzMF.Coat3Texture])")
//                    .opacity(1)
//                Rectangle()
//                    .fill(Color.black)
//                    .opacity(0.9)
...
//            .background(Color.gray.opacity(0.2))
//            .cornerRadius(5)
//            .onChange(of: QuartzMF.Coat3Texture) { _ in
//                checkTexture()
//            }
            ...
            VStack { // Start of coat summary
                ...
                if QuartzMF.Coat3PtA == 4 {
                    HStack{
                        Text("\(typesACodes[QuartzMF.Coat3PtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: Ench. Green, 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzMF_TSAC3))")
                            .font(.caption)
                    }
                }
                else {
                    HStack{
                        Text("\(typesACodes[QuartzMF.Coat3PtA])")
                            .font(.caption)
                        Spacer()
                        Text("TSE Part A: \(typesA[QuartzMF.Coat3PtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzMF_TSAC3))")
                            .font(.caption)
                ...
                HStack{
                    Text("\(typesBCodes[QuartzMF.Coat3PtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzMF.Coat3PtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzMF_TSAC3))")
                        .font(.caption)
                }
                if QuartzMF.Coat3TSColorant != 0 {

                        HStack {
                            Text("\(TSColorantCodes[QuartzMF.Coat3TSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzMF.Coat3TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzMF_TSColorantC3))")
                                .font(.caption)
                        }
            ...
            else if QuartzMF.Coat3ProductType == 1
            {
                HStack
                {
                    Text("\(PCoatPtACode)")
                        .font(.caption)
                    Spacer()
                    Text("\(PATypes[QuartzMF.Coat3PtA]) - \(hardeners[QuartzMF.Coat3PtB]), 1 gal ")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzMF_TSAC3))")
                        .font(.caption)
                ...
                HStack
                {
                    Text("\(PCoatHardenerCode)")
                        .font(.caption)
                    Spacer()
                    Text("\(PATypes[QuartzMF.Coat3PtA]) Part B, 1 gal ")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: QuartzMF_TSAC3))")
                        .font(.caption)
            ...
            else
            {
                ...
                HStack
                {
                    ...
                    if QuartzMF.Coat3PtA == 0
                    {
                    Text("\(PUTypes[QuartzMF.Coat3PtA]) - \(hardeners[QuartzMF.Coat3PtB]) Part A, 1 gal ")
                        .font(.caption)
                    }
                    // Poly HP Gloss
                    else if QuartzMF.Coat3PtA == 1
                    {
                    Text("\(PUTypes[QuartzMF.Coat3PtA]) Part A, 1/4 gal ")
                        .font(.caption)
                    }
                    // Poly HP Satin
                    else
                    {
                        Text("\(PUTypes[QuartzMF.Coat3PtA]) Part A, 1/2 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzMF_TSAC3))")
                ...
                HStack
                {
                    ...
                    if QuartzMF.Coat3PtA == 0
                    {
                    Text("\(PUTypes[QuartzMF.Coat3PtA]) Part B, 1/2 gal ")
                        .font(.caption)
                    }
                    else
                    {
                        Text("\(PUTypes[QuartzMF.Coat3PtA]) Part B, 1 gal ")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: QuartzMF_TSAC3))")
                ...
                // poly colorant
                if QuartzMF.Coat3PColorant != 0 && QuartzMF.Coat3ProductType != 0
                {
                    HStack
                    {
                        Text("\(PolyColorantCodes[QuartzMF.Coat3PColorant])")
                            .font(.caption)
                        Spacer()
                       
                        Text("Poly Colorant - \(PolyColorantChoices[QuartzMF.Coat3PColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzMF_TSColorantC3))")
                            .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzMF.Coat3Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: QuartzMF_TSAC3) + QuartzMF.Coat3Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct QuartzMFCoat3_Previews: PreviewProvider {
