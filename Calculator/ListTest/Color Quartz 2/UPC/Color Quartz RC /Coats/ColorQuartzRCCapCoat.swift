...
var QuartzRC_TSAC3 = stats2(product: "Top Shelf® Epoxy Part A:", covRate: 75, MixRat: 1)
var QuartzRC_TSBC3 = stats2(product: "Top Shelf® Epoxy Part B:", covRate: 75, MixRat: 0.5)
var QuartzRC_TSColorantC3 = stats2(product: "Top Shelf® Epoxy Colorant:", covRate: 75, MixRat: 16)


struct ColorQuartzRCCapCoat: View {
...
//        {
//            QuartzRC_TSAC3.covRate = 120
//            QuartzRC_TSBC3.covRate = 120
//        }
//        // change cov rate to 800 if polyA is chosen
//        else if QuartzRC.Coat3ProductType == 1
//        {
//            QuartzRC_TSAC3.covRate = 160
//            QuartzRC_TSBC3.covRate = 160
//        }
//        // else poly is chosen
//        else
//        {
...
//    }
    ...
    func checkPCoatCodes() -> Void {
        ...
        if QuartzRC.Coat3ProductType == 1
        {
            if QuartzRC.Coat3PtA == 0 && QuartzRC.Coat3PtB == 0 {
                QuartzRC.Coat3PtACode = "EX-KPLY72AZ-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY72B-01"
            }
            else if QuartzRC.Coat3PtA == 0 && QuartzRC.Coat3PtB == 1 {
                QuartzRC.Coat3PtACode = "EX-KPLY72AF-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY72B-01"
                
            }
            else if QuartzRC.Coat3PtA == 1 && QuartzRC.Coat3PtB == 0 {
                QuartzRC.Coat3PtACode = "EX-KPLY85AZ-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY85B-01"
            }
            else if QuartzRC.Coat3PtA == 1 && QuartzRC.Coat3PtB == 1 {
                QuartzRC.Coat3PtACode = "EX-KPLY85AF-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY85B-01"
                
            }
            else if QuartzRC.Coat3PtA == 2 && QuartzRC.Coat3PtB == 0 {
                QuartzRC.Coat3PtACode = "EX-KPLY92AZ-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY92B-01"
                
            }
            else if QuartzRC.Coat3PtA == 2 && QuartzRC.Coat3PtB == 1 {
                QuartzRC.Coat3PtACode = "EX-KPLY92AF-01"
                QuartzRC.Coat3PtBCode = "EX-KPLY92B-01"
        ...
        else if QuartzRC.Coat3ProductType == 2
        {
            ...
            // POLY HS EZ
            if QuartzRC.Coat3PtA == 0 && QuartzRC.Coat3PtB == 0 {
                QuartzRC.Coat3PtACode = "EX-KPLYHSAZ-01"
                QuartzRC.Coat3PtBCode = "EX-KPLYHSB-01"
            }
            // POLY HS FAST
            else if QuartzRC.Coat3PtA == 0 && QuartzRC.Coat3PtB == 1 {
                QuartzRC.Coat3PtACode = "EX-KPLYHSAF-01"
                QuartzRC.Coat3PtBCode = "EX-KPLYHSB-01"
    ...
    var body: some View {
        VStack {
            HStack {
                Text("Coat 3 Product:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzRC.Coat3ProductType,
                       label: ZStack {
                    Text("\(products[QuartzRC.Coat3ProductType])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                    checkPUCodes()
 ...
 //               QuartzRCInfo()
            ...
            HStack {
                Toggle(isOn: $QuartzRC.Coat3MatAdd, label: {
            ...
            Text("Top Shelf Epoxy® Part A:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat3PtA,
                   label: ZStack {
                Text("\(typesA[QuartzRC.Coat3PtA])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.black)
                    .opacity(0.9)
            ...
            Text("Top Shelf Epoxy® Part B:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat3PtB,
                   label: ZStack {
                Text("\(typesB[QuartzRC.Coat3PtB])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            .background(Color.gray.opacity(0.2))
            .cornerRadius(5)
            //             .onChange(of: QuartzRC.Coat3PtB) { _ in
            //                 CheckCoatChoices()
            //             }
            Text("Top Shelf Epoxy® Colorant:")
                .fontWeight(.bold)
            Picker(selection: $QuartzRC.Coat3TSColorant,
                   label: ZStack {
                Text("\(TSColorantChoices[QuartzRC.Coat3TSColorant])")
                    .opacity(1)
                Rectangle()
                    .fill(Color.gray)
                    .opacity(0.2)
            ...
            else if QuartzRC.Coat3ProductType == 1
            {
                ...
                .background(Color.gray.opacity(0.2))
                .cornerRadius(5)
                .onChange(of: QuartzRC.Coat3PtA) { _ in
                    checkPCoatCodes()
                }
                
                Picker(selection: $QuartzRC.Coat3PtB,
                       label: ZStack {
                    Text("\(hardeners[QuartzRC.Coat3PtB])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//                        checkPUCodes()
            ...
            if QuartzRC.Coat3ProductType != 0
            {
                Text("Poly Colorant:")
                    .fontWeight(.bold)
                Picker(selection: $QuartzRC.Coat3PColorant,
                       label: ZStack {
                    Text("\(PolyColorantChoices[QuartzRC.Coat3PColorant])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.black)
                        .opacity(0.9)
...
//            Text("Texture:")
//                .fontWeight(.bold)
//            Picker(selection: $QuartzRC.Coat3Texture,
//                   label: ZStack {
//                Text("\(TextureOptions[QuartzRC.Coat3Texture])")
//                    .opacity(1)
//                Rectangle()
//                    .fill(Color.black)
//                    .opacity(0.9)
...
//            .background(Color.gray.opacity(0.2))
//            .cornerRadius(5)
//            .onChange(of: QuartzRC.Coat3Texture) { _ in
//                checkTexture()
//            }
                ...
                if QuartzRC.Coat3PtA == 4 {
                    HStack{
                        ...
                        Text("TSE Part A: Ench. Green, 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: QuartzRC_TSAC3))")
                            .font(.caption)
                    }
                }
                else {
                ...
                if QuartzRC.Coat3TSColorant != 0 {
                        ...
                        HStack {
                            ...
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzRC.Coat3TSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: QuartzRC_TSColorantC3))")
                                .font(.caption)
                        }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzRC.Coat3Waste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: QuartzRC_TSAC3) + QuartzRC.Coat3Waste) kit(s)")
            }
            .padding()
        }
    }
...
struct QuartzRCCoat3_Previews: PreviewProvider {
