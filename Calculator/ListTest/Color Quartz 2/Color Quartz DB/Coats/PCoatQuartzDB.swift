...
struct PCoatQuartzDB: View {
    ...
    @EnvironmentObject var TSColorant_QuartzDB : stats //Used for system stats (mixRat,CovRate, etc)
    ...
    var body: some View {
        VStack {
           ...
           Text("Top Shelf Epoxy® Part A:")
            .fontWeight(.bold)
            Picker(selection: $QuartzDB.PCoatPtA,
                label: ZStack {
                    Text("\(typesA[QuartzDB.PCoatPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.PCoatPtB,
                 label: ZStack {
                     Text("\(typesB[QuartzDB.PCoatPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: QuartzDB.PCoatPtB) { _ in
//                quant(product: TSB)
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.PCoatTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[QuartzDB.PCoatTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            VStack { // Start of coat summary
                HStack {
                    ...
                    HStack{
                        ...
                        Text("TSE Part A: \(typesA[QuartzDB.PCoatPtA]), 1 gal")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSA_QuartzDB))")
                            .font(.caption)
                    }
                ...
                HStack{
                    ...
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzDB.PCoatPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSB_QuartzDB))")
                        .font(.caption)
                }
                ...
                if QuartzDB.PCoatTSColorant != 0
                {
                    HStack {
                        ...
                        Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[QuartzDB.BCoatTSColorant]), 16 oz")
                            .font(.caption)
                        Spacer()
                        Text("\(quant(product: TSColorant_QuartzDB))")
                            .font(.caption)
                    }
                }
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzDB.PCoatWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSA_QuartzDB) + QuartzDB.PCoatWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct PCoatQuartzDB_Previews: PreviewProvider {
