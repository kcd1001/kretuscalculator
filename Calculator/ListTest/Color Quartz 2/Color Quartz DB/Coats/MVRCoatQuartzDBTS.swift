...
struct MVRCoatQuartzDB: View {
    ...
    var body: some View {
        VStack {
            ...
            .fontWeight(.bold)
            Picker(selection: $QuartzDB.MVRPtA,
                label: ZStack {
                    Text("\(typesA[QuartzDB.MVRPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            .cornerRadius(5)
//            .onChange(of: QuartzDB.PCoatPtA) { _ in
//                quant(product: TSA_TS)
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.MVRPtB,
                 label: ZStack {
                     Text("\(typesB[QuartzDB.MVRPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $QuartzDB.MVRTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[QuartzDB.MVRTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: QuartzDB.PCoatTSColorant) { _ in
...
//             }
            VStack { // Start of coat summary
                ...
                HStack{
                    Text("\(typesBCodes[QuartzDB.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[QuartzDB.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr))")
                        .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $QuartzDB.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSMvr) + QuartzDB.MVRWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct MVRCoatQuartzDB_Previews: PreviewProvider {
