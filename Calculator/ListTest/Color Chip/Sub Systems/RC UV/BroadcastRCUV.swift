...
//    var Broadcast = stats(product: "Color Chip:", covRate: 0.15, MixRat: 55)
    ...
    func checkChipSize() {
        ...
        else if ChipRCUV.BroadcastSizeSelection == 1 {
            Broadcast.covRate = 0.15
        }
    }
    ...
    var body: some View {
        VStack {
            ...
            Text("Color Chip:")
             .fontWeight(.bold)
             Picker(selection: $ChipRCUV.BroadcastSizeSelection, //used for size of chips
                 label: ZStack {
                     Text("\(ChipSizes[ChipRCUV.BroadcastSizeSelection])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
*/
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $ChipRCUV.BroadcastWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: Broadcast) + ChipRCUV.BroadcastWaste) box(es)")
            }
            .padding()
        }
    }
...
struct BroadcastRCUV_Previews: PreviewProvider {
