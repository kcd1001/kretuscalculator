...
struct MVRCoatRCUV: View {
    ...
    var body: some View {
        VStack {
            Text("UPC Colorant:")
             .fontWeight(.bold)
             Picker(selection: $ChipRCUV.MVRCoatColor,
                 label: ZStack {
                     Text("\(UPCColorantChoices[ChipRCUV.MVRCoatColor])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $ChipRCUV.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: UPCA) + ChipRCUV.MVRWaste) kit(s)")
            }
    }
            .padding()
        }
...
struct MVRCoatRCUV_Previews: PreviewProvider {
