...
struct MVRCoatTS: View {
    ...
    var body: some View {
        VStack {
            ...
            .fontWeight(.bold)
            Picker(selection: $ChipTS.MVRPtA,
                label: ZStack {
                    Text("\(typesA[ChipTS.MVRPtA])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            .cornerRadius(5)
//            .onChange(of: ChipTS.PCoatPtA) { _ in
//                quant(product: TSA_TS)
//            }
            Text("Top Shelf Epoxy® Part B:")
             .fontWeight(.bold)
             Picker(selection: $ChipTS.MVRPtB,
                 label: ZStack {
                     Text("\(typesB[ChipTS.MVRPtB])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
...
//             }
            Text("Top Shelf Epoxy® Colorant:")
             .fontWeight(.bold)
             Picker(selection: $ChipTS.MVRTSColorant,
                 label: ZStack {
                     Text("\(TSColorantChoices[ChipTS.MVRTSColorant])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .background(Color.gray.opacity(0.2))
             .cornerRadius(5)
//             .onChange(of: ChipTS.PCoatTSColorant) { _ in
...
//             }
            VStack { // Start of coat summary
                ...
                HStack{
                    Text("\(typesBCodes[ChipTS.MVRPtB])")
                        .font(.caption)
                    Spacer()
                    Text("Top Shelf® Epoxy Part B: \(typesB[ChipTS.MVRPtB]), 1/2 gal")
                        .font(.caption)
                    Spacer()
                    Text("\(quant(product: TSMvr))")
                        .font(.caption)
                ...
                if ChipTS.MVRTSColorant != 0 {
                        HStack {
                            Text("\(TSColorantCodes[ChipTS.MVRTSColorant])")
                                .font(.caption)
                            Spacer()
                            Text("Top Shelf® Epoxy Colorant:  \(TSColorantChoices[ChipTS.MVRTSColorant]), 16 oz")
                                .font(.caption)
                            Spacer()
                            Text("\(quant(product: TSMvr))")
                                .font(.caption)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $ChipTS.MVRWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quant(product: TSMvr) + ChipTS.MVRWaste) kit(s)")
            }
            .padding()
        }
    }
...
struct MVRCoatTS_Previews: PreviewProvider {
