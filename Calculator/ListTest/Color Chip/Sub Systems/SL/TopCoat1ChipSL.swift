...
struct TopCoat1ChipSL: View {
    ...
    func checkTC2()  {
        if ChipSL.TCoat2Choice == 3 { //POLY HS
            PAPUTC2.covRate = 560
        }
        else if ChipSL.TCoat2Choice == 4 { //POLY HP CLEAR GLOSS
            PAPUTC2.covRate = 575
        }
        else if ChipSL.TCoat2Choice == 5 { //POLY HP CLEAR SATIN
            PAPUTC2.covRate = 675
        }
        else {
            PAPUTC2.covRate = 750 
    ...
    func checkTC1() { // change tc1 product coverage rate based off selection and chip size 
        if ChipSL.BroadcastSizeSelection == 0 {
            switch ChipSL.TCoat1Choice {
            case 3:
                PAPUTC1.covRate = 210
            default:
                PAPUTC1.covRate = 300
    ...
    var body: some View {
        ...
        Text("Top Coat 1:")
         .fontWeight(.bold)
        HStack {
             Picker(selection: $ChipSL.TCoat1Choice,
                 label: ZStack {
                     Text("\(TopCoat1Options[ChipSL.TCoat1Choice])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
            ...
            Picker(selection: $ChipSL.TCoat1Hardener,
                label: ZStack {
                    Text("\(TCoat1HardenerOptions[ChipSL.TCoat1Hardener])")
                        .opacity(1)
                    Rectangle()
                        .fill(Color.gray)
                        .opacity(0.2)
            ...
            Text("Top Coat 2:")
                .fontWeight(.bold)
            HStack {
                 Picker(selection: $ChipSL.TCoat2Choice,
                     label: ZStack {
                         Text("\(TopCoat2Options[ChipSL.TCoat2Choice])")
                             .opacity(1)
                         Rectangle()
                             .fill(Color.gray)
                             .opacity(0.2)
                ...
                if ChipSL.TCoat2Choice != 6 && ChipSL.TCoat2Choice != 4 && ChipSL.TCoat2Choice != 5 {
                Picker(selection: $ChipSL.TCoat2Hardener,
                    label: ZStack {
                        Text("\(TCoat2HardenerOptions[ChipSL.TCoat2Hardener])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            if ChipSL.TCoat2Choice != 6 { //Displays texture if it is selected
            Text("Texture:")
             .fontWeight(.bold)
                Picker(selection: $ChipSL.texture1,
                    label: ZStack {
                        Text("\(TextureOptions[ChipSL.texture1])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            VStack { // Start of coat summary
                ...
                if (ChipSL.TCoat1Choice != ChipSL.TCoat2Choice) {
                    ...
                    Text("\(TCoat1PtACode)")
                        .font(.caption)
                    Spacer()
                    Text("\(TopCoat1Options[ChipSL.TCoat1Choice]) - \(TCoat1HardenerOptions[ChipSL.TCoat1Hardener]), 1 gal")
                        .font(.caption)
                    Spacer()
                ...
                HStack {
                    ...
                    Text("\(TopCoat1Options[ChipSL.TCoat1Choice]) Part B, 1 gal")
                        .font(.caption)
                    }
                    else {
                        Text("\(TopCoat1Options[ChipSL.TCoat1Choice]) Part B, 1/2 gal")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: PAPUTC1))")
                ...
                HStack {
                    Text("\(TCoat2PtACode)")
                        .font(.caption)
                    Spacer()
                    Text("\(TopCoat2Options[ChipSL.TCoat2Choice]) - \(TCoat2HardenerOptions[ChipSL.TCoat2Hardener]), 1 gal")
                        .font(.caption)
                    Spacer()
                ...
                HStack {
                    ...
                    Spacer()
                    if ChipSL.TCoat2Choice != 3 {
                    Text("\(TopCoat2Options[ChipSL.TCoat2Choice]) Part B, 1 gal")
                        .font(.caption)
                    }
                    else {
                        Text("\(TopCoat2Options[ChipSL.TCoat2Choice]) Part B, 1/2 gal")
                            .font(.caption)
                    }
                    Spacer()
                    Text("\(quant(product: PAPUTC2))")
        ...
        HStack {
            ...
            Spacer()
            TextField("",value: $ChipSL.TCoatWaste, formatter: NumberFormatter())
        ...
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + quant(product: PAPUTC2) + ChipSL.TCoatWaste) kit(s)")
        }
        }
        else {
        HStack {
            Text("Total:")
            Spacer()
            Text("\(quant(product: PAPUTC1) + ChipSL.TCoatWaste) kit(s)")
        }
        }
        }
    }
...
struct TopCoat1ChipSL_Previews: PreviewProvider {
