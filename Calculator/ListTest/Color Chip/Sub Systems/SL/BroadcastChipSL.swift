...
struct BroadcastChipSL: View {
    ...
    func checkChipSize() {
        ...
        else if ChipSL.BroadcastSizeSelection == 1 {
    ...
    var body: some View {
        VStack {
            ...
            Text("Color Chip:")
             .fontWeight(.bold)
             Picker(selection: $ChipSL.BroadcastSizeSelection, //used for size of chips
                 label: ZStack {
                     Text("\(ChipSizes[ChipSL.BroadcastSizeSelection])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
 ...
 */
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $ChipSL.BroadcastWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: Broadcast) + ChipSL.BroadcastWaste) box(es)")
            }
            .padding()
        }
    }
...
struct BroadcastChipSL_Previews: PreviewProvider {
