...
//    var Broadcast = stats(product: "Color Chip:", covRate: 0.15, MixRat: 55)
    ...
    func checkChipSize() {
        ...
        else if ChipRC.BroadcastSizeSelection == 1 {
            Broadcast.covRate = 0.15
        }
    }
    ...
    var body: some View {
        VStack {
            ...
            Text("Color Chip:")
             .fontWeight(.bold)
             Picker(selection: $ChipRC.BroadcastSizeSelection, //used for size of chips
                 label: ZStack {
                     Text("\(ChipSizes[ChipRC.BroadcastSizeSelection])")
                         .opacity(1)
                     Rectangle()
                         .fill(Color.gray)
                         .opacity(0.2)
             ...
             .onChange(of: ChipRC.BroadcastSizeSelection) { _ in
                ...
                Picker(selection: $ChipRC.BroadcastSelection,
                    label: ZStack {
                        Text("\(ChipChoices[ChipRC.BroadcastSelection])")
                            .opacity(1)
                        Rectangle()
                            .fill(Color.gray)
                            .opacity(0.2)
            ...
            HStack {
                ...
                Spacer()
                TextField("",value: $ChipRC.BroadcastWaste, formatter: NumberFormatter())
            ...
            HStack {
                Text("Total:")
                Spacer()
                Text("\(quantBroadcast(product: Broadcast) + ChipRC.BroadcastWaste) box(es)")
            }
            .padding()
        }
    }
...
struct BroadcastChipRC_Previews: PreviewProvider {
